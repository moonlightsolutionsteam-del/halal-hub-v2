        ...
        <TabsContent value="dashboard" className="space-y-8 m-0 animate-in fade-in duration-500">
          ...
          <Card className="rounded-[2rem] border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="p-8 flex flex-row items-center justify-between">
              <div className="space-y-1">
                <CardTitle className="text-xl font-black text-slate-900">Recent Sourcing Logs</CardTitle>
                <p className="text-sm text-muted-foreground font-medium">Verify incoming slaughterhouse certificates and batch proofs.</p>
              </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="p-8 border-b space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative w-full md:w-96">
                  ...
                  <Input placeholder="Search shops by name, city or license..." className="pl-9 h-12 rounded-2xl bg-slate-50 border-none font-medium" />
                </div>
            ...
            <CardContent className="p-0">
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Shop Identity</TableHead>
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Location</TableHead>
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Trust Score</TableHead>
                  ...
                  {MOCK_SHOPS.map((shop) => (
                    <TableRow key={shop.id} className="border-slate-100 hover:bg-slate-50/50 transition-colors group">
                      <TableCell className="px-8 py-5">
                        ...
                        <p className="text-[10px] font-bold text-primary uppercase">{shop.id}</p>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={
                          shop.status === 'Verified' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-amber-50 text-amber-600 border-amber-200'
                        }>
                          {shop.status}
                        </Badge>
                      </TableCell>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-10">
            <div className="space-y-10">
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground font-medium text-lg">Define mandatory compliance checks for all meat merchants.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-transparent hover:border-primary/20 transition-all">
                      ...
                      <Badge className={rule.required ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}>
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white space-y-6 relative overflow-hidden">
                  ...
                  <div className="space-y-2 relative z-10">
                    <h4 className="text-xl font-black">Audit Frequency</h4>
                    <p className="text-slate-400 text-sm">Automated reminders for certificate renewals.</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4 relative z-10">
                    <Button variant="secondary" className="rounded-xl font-bold bg-white/10 hover:bg-white/20 border-none text-white">Every 3 Months</Button>
                    <Button variant="secondary" className="rounded-xl font-bold bg-primary text-white border-none shadow-lg">Every 6 Months</Button>
                  </div>
                </div>
              </div>
            </div>
        ...
        {["reviews", "offers", "loyalty", "certificates", "categories"].map((tab) => (
          <TabsContent key={tab} value={tab} className="animate-in fade-in duration-500 m-0">
            <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-20 text-center space-y-6">
              ...
              <Button variant="outline" className="rounded-xl border-2 font-bold px-8">Sync Console</Button>
            </Card>
          </TabsContent>
        ))}
      </Tabs>
