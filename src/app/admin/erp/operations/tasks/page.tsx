...
export default function OperationsTasksPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold font-headline">Operations Tasks</h1>
        <p className="text-muted-foreground">Manage day-to-day operational tasks and assignments.</p>
      </div>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Done", value: tasks.filter(t => t.status === "Done" || t.status === "Completed").length },
        ].map(({ label, value }) => (
          <Card key={label}>
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                <DialogHeader><DialogTitle>Create Operations Task</DialogTitle></DialogHeader>
                ...
                {filtered.map(t => (
                  <TableRow key={t.id}>
                    <TableCell>
                      ...
                      {t.description && <div className="text-xs text-muted-foreground">{t.description.slice(0, 60)}{t.description.length > 60 ? "…" : ""}</div>}
                    </TableCell>
