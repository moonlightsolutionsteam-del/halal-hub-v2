...
import { 
  ...
  CookingPot, Utensils, Truck, Timer, ClipboardList
} from "lucide-react"
import {
  Select,
  SelectContent,
...
export default function SuperAdminCateringManagement() {
  ...
  const MOCK_CATERERS = [
    { id: "CAT-001", name: "Elite Halal Catering", city: "Manhattan, NY", type: "Full Service", status: "Verified", rating: 4.9, events: 124 },
    { id: "CAT-002", name: "Sunnah Sun-Sets", city: "Brooklyn, NY", type: "Boutique", status: "Verified", rating: 4.8, events: 85 },
    { id: "CAT-003", name: "Grand Dastarkhwan", city: "Queens, NY", type: "Traditional", status: "Audit Needed", rating: 4.7, events: 210 },
    { id: "CAT-004", name: "Modern Mezze Co.", city: "Jersey City, NJ", type: "Corporate", status: "Verified", rating: 4.5, events: 45 },
  ];

  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto pb-24">
      <div className="space-y-1">
        <h1 className="text-2xl sm:text-3xl font-black font-headline text-foreground uppercase tracking-tighter">Catering Services</h1>
        <p className="text-muted-foreground font-medium text-sm sm:text-lg italic">Manage event caterers, logistics transparency, and supply chain audits.</p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
        ...
        <TabsContent value="dashboard" className="space-y-8 m-0 animate-in fade-in duration-500">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-6">
            <Card className="rounded-[2rem] border-none shadow-sm bg-card p-8 group hover:shadow-md transition-all">
              ...
              <div className="space-y-1">
                <p className="text-2xl sm:text-4xl font-black text-foreground">120</p>
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-card p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Verified Logistics</span>
                <div className="h-10 w-10 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600">
                  <ShieldCheck className="h-5 w-5" />
                </div>
              </div>
              <div className="space-y-1">
                <p className="text-2xl sm:text-4xl font-black text-foreground">95</p>
                <p className="text-[10px] font-bold text-muted-foreground uppercase">79% audited</p>
              </div>
            </Card>

            <Card className="rounded-[2rem] border-none shadow-sm bg-card p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Avg. Event Value</span>
                <div className="h-10 w-10 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600">
                  <Landmark className="h-5 w-5" />
                </div>
              </div>
              <div className="space-y-1">
                <p className="text-2xl sm:text-4xl font-black text-foreground">₹85k</p>
                <p className="text-[10px] font-bold text-muted-foreground uppercase">per platform booking</p>
              </div>
            </Card>
          </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card">
            <CardHeader className="p-8 border-b space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative w-full md:w-96">
                  ...
                  <Input placeholder="Search caterers by name, ID or specialty..." className="pl-9 h-12 rounded-2xl bg-muted border-none font-medium" />
                </div>
            ...
            <CardContent className="p-0">
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black text-[10px] uppercase tracking-widest text-muted-foreground">Partner Identity</TableHead>
                    ...
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-center">Total Events</TableHead>
                ...
                <TableBody>
                  {MOCK_CATERERS.map((cat) => (
                    <TableRow key={cat.id} className="border-border hover:bg-muted/50 transition-colors group">
                      <TableCell className="px-8 py-5">
                        <p className="font-black text-foreground text-base">{cat.name}</p>
                        <p className="text-[10px] font-bold text-primary uppercase tracking-tighter">{cat.id} • {cat.type}</p>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center justify-center gap-1.5 font-black text-sm text-amber-500">
                          <Star className="h-3.5 w-3.5 fill-current border-none" /> {cat.rating}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={
                          cat.status === 'Verified' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 font-black text-[9px] px-3' : 'bg-amber-50 text-amber-600 border-amber-200 font-black text-[9px] px-3'
                        }>
                          {cat.status}
                        </Badge>
                      </TableCell>
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card">
                ...
                <CardContent className="p-0">
                  <Table>
                    <TableBody>
                      {[
                        { name: "Elite Central Kitchen", docs: 5, date: "2 mins ago", type: "Renewal" },
                        { name: "Royal Feast HQ", docs: 3, date: "45 mins ago", type: "Initial Audit" },
                        { name: "Modern Mezze Hub", docs: 8, date: "3 hours ago", type: "Compliance" },
                      ].map((item, i) => (
                        <TableRow key={i} className="border-border hover:bg-muted/50 transition-colors">
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-zinc-900 text-white p-8 space-y-6 relative overflow-hidden">
                ...
                <div className="space-y-2 relative z-10">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed italic">
                    12 logistics chain audits have been in the queue for more than 48 hours. Fresh catering deployments are prioritized.
                  </p>
                </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-10">
            <div className="space-y-10">
              <div className="space-y-2 border-b pb-6">
                <h3 className="text-3xl font-black text-foreground">Off-site Compliance Standards</h3>
                <p className="text-muted-foreground font-medium text-sm sm:text-lg italic">Define protocols for logistics and base kitchen management.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="bg-zinc-900 rounded-[2.5rem] p-10 text-white space-y-6 relative overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="space-y-2 relative z-10">
                    ...
                    <p className="text-muted-foreground text-sm">Automated tracking of catering vehicle thermal logs.</p>
                  </div>
                  <Button variant="secondary" className="w-full rounded-xl font-black text-[10px] h-12 uppercase tracking-widest relative z-10 shadow-xl bg-card text-foreground">Enable Thermal Monitor</Button>
                </div>
              </div>
            </div>
          </Card>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card">
            <CardHeader className="p-8 border-b flex flex-row items-center justify-between bg-amber-50/10">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground font-medium italic">Moderate event reviews specifically focused on setup and quality.</p>
              </div>
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow className="border-none">
                    ...
                    <TableHead className="h-14 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Partner & Event</TableHead>
                ...
                <TableBody>
                  {[
                    { id: "REV-CAT-901", partner: "Elite Catering", event: "Nikah Gala", reason: "Setup Delay", date: "2h ago" },
                    { id: "REV-CAT-902", partner: "Sunnah Sets", event: "Corporate Lunch", reason: "Temp Issue", date: "5h ago" },
                  ].map((rep, i) => (
                    <TableRow key={i} className="border-border hover:bg-muted/50 transition-colors group">
                      ...
                      <TableCell>
                        <p className="font-black text-foreground text-base">{rep.event}</p>
                        <p className="text-[10px] font-bold text-primary uppercase">by {rep.partner}</p>
                      </TableCell>
                      <TableCell className="text-center">
        ...
        <TabsContent value="offers" className="animate-in fade-in duration-500 m-0">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { title: "Wedding Early Bird", code: "WEDDING20", discount: "20% OFF", active: true, claims: 12 },
            ...
            <button className="rounded-[2.5rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-primary/20 transition-all cursor-pointer group min-h-[300px]">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground font-medium">Create seasonal or event bulk deals</p>
              </div>
            </button>
          </div>
        </TabsContent>
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-primary text-primary-foreground p-10 relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-8">
                  <div className="space-y-2">
                    ...
                    <h2 className="text-4xl sm:text-7xl font-black tracking-tighter">64.2%</h2>
                    <div className="flex items-center gap-2 text-sm font-bold bg-card/20 w-fit px-4 py-1.5 rounded-full backdrop-blur-md">
                      <TrendingUp className="h-4 w-4" /> +12% Retention Lift
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 border-t border-white/10 pt-8">
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase opacity-60 tracking-widest mb-1">Catering Coins Issued</p>
            ...
            <div className="lg:col-span-4 space-y-8">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-8 space-y-6 flex flex-col justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground font-medium italic">Manage how clients earn Hub Coins for catering contracts.</p>
                </div>
                <div className="space-y-4">
                  {[
                    { label: "Standard Earn", rate: "1 Coin / ₹100" },
                    { label: "Verified Partner Multiplier", rate: "1.2x Multiplier" },
                    { label: "Corporate Volume Bonus", rate: "500 Flat Coins" },
                  ].map((tier, i) => (
            ...
            <Card className="lg:col-span-2 rounded-[2.5rem] border-none shadow-sm bg-card">
              <CardHeader className="p-8 border-b bg-muted/30 flex flex-row items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground font-medium italic">Track physical and digital certification distribution for catering teams.</p>
                </div>
              ...
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {[
                      { name: "Elite Halal Catering", type: "Digital Badge + QR Kit", status: "Active", date: "Nov 01" },
                      { name: "Sunnah Sun-Sets", type: "Kitchen Seal", status: "In Transit", date: "Oct 28" },
                      { name: "Modern Mezze Co.", type: "Physical Plaque", status: "Active", date: "Oct 15" },
                    ].map((item, i) => (
                      <TableRow key={i} className="border-border hover:bg-muted/50 transition-colors">
                        <TableCell className="px-8 py-5 font-bold text-foreground text-sm">{item.name}</TableCell>
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-8 space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { label: "Kitchen Seals", count: 80 },
                    { label: "Audit Certificates", count: 120 },
                  ].map((inv, i) => (
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card">
            <CardHeader className="p-8 border-b flex flex-row items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground font-medium italic">Control primary categories and listing tags for the catering vertical.</p>
              </div>
            ...
            <CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  {[
                    { name: "Full Service Events", count: 42, trend: "Steady" },
                    { name: "Corporate Boxed Meals", count: 28, trend: "Rising" },
                    { name: "Live Counter Stations", count: 15, trend: "High Demand" },
                    { name: "Traditional Bulk (Deg)", count: 35, trend: "Stable" },
                  ].map((cat, i) => (
                    <TableRow key={i} className="border-border hover:bg-muted/50 transition-colors group">
                      <TableCell className="px-8 py-5 font-black text-foreground text-sm uppercase tracking-tight">{cat.name}</TableCell>
                      <TableCell className="font-bold text-muted-foreground text-xs">{cat.count} establishments</TableCell>
                      <TableCell className="text-center">
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-10 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="p-8 bg-muted rounded-[2rem] space-y-2 border shadow-inner">
                    ...
                    <p className="text-3xl font-black text-foreground">5.0% <span className="text-sm font-bold text-muted-foreground italic">flat</span></p>
                    <p className="text-xs font-bold text-emerald-600">On all digital event bookings</p>
                  </div>
                  <div className="p-8 bg-muted rounded-[2rem] space-y-2 border shadow-inner">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Net Revenue (MTD)</p>
                    <p className="text-3xl font-black text-foreground">₹4.2M</p>
                    <p className="text-xs font-bold text-muted-foreground uppercase">Vertical gross volume</p>
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="divide-y border rounded-[2rem] overflow-hidden">
                    {[
                      { id: "SET-CAT-991", merchant: "Elite Catering", amount: "₹1,45,000", status: "Released", date: "Nov 01" },
                      { id: "SET-CAT-992", merchant: "Sunnah Sets", amount: "₹45,200", status: "Processing", date: "Oct 30" },
                    ].map((set, i) => (
