...
import { 
  ...
  AlertTriangle, FlaskConical, Scale
} from "lucide-react"
import { 
    ...
    <div className="container mx-auto p-6 space-y-10 max-w-7xl pb-24 text-slate-900">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-black font-headline text-slate-900 tracking-tight">Audit & Integrity Portal</h1>
        ...
        <div className="flex gap-3">
          <Button variant="outline" className="rounded-2xl px-6 font-black border-2 h-14 bg-white shadow-sm gap-2">
            <History className="h-4 w-4 text-slate-400" /> Audit History
          </Button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <Card className="lg:col-span-8 rounded-[3rem] border-none shadow-sm bg-white overflow-hidden">
              <CardHeader className="p-10 border-b bg-slate-50/30 flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="rounded-xl font-black text-xs h-10 px-6 border-2 bg-white">Full Registry</Button>
              </CardHeader>
              <CardContent className="p-0">
                <Table>
                  <TableHeader className="bg-slate-50/50">
                    <TableRow className="border-none">
                      <TableHead className="px-10 h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Client / ID</TableHead>
                      <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Vertical</TableHead>
                      ...
                      <TableRow key={i} className="border-slate-50 hover:bg-slate-50/50 transition-colors group">
                        <TableCell className="px-10 py-6">
                          <div className="flex items-center gap-4">
                            <div className="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center font-black text-xs text-slate-400">
                              {client.name[0]}
                            </div>
                            <div>
                              <p className="font-black text-slate-900 text-sm tracking-tight">{client.name}</p>
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">ID: {client.id}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge variant="secondary" className="bg-slate-50 text-slate-600 border-none font-black text-[9px] uppercase px-3 h-6 flex items-center w-fit">{client.vertical}</Badge>
                        </TableCell>
                        <TableCell>
                          <Badge className={cn(
            ...
            <div className="lg:col-span-4 space-y-8">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-slate-900 text-white p-10 space-y-8 relative overflow-hidden flex flex-col justify-between h-full">
                ...
                <div className="relative z-10 space-y-6">
                  <div className="h-16 w-16 bg-white/10 rounded-2xl flex items-center justify-center text-primary shadow-xl border border-white/10">
        ...
        {["clients", "audits", "certificates", "reports", "billing"].map((tab) => (
          <TabsContent key={tab} value={tab} className="animate-in fade-in duration-500 m-0">
            <Card className="rounded-[3rem] border-none shadow-sm bg-white p-32 text-center space-y-8">
              ...
              <Button variant="outline" className="rounded-2xl border-2 font-black px-12 h-14 uppercase text-xs tracking-[0.2em] hover:bg-slate-50">Refresh Dataset</Button>
            </Card>
          </TabsContent>
        ))}
      </Tabs>
