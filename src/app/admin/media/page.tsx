...
export default function SuperAdminMediaManagement() {
  ...
  const cat = useAdminCategory("Media & Publishing")

  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto pb-24">
      <div className="space-y-1">
        <h1 className="text-2xl sm:text-3xl font-black font-headline text-foreground uppercase tracking-tighter text-muted-foreground">Bookstores & Islamic Media</h1>
        <p className="text-muted-foreground font-medium text-sm sm:text-lg italic">Manage publishers, digital media integrity, and scholarly vetting.</p>
      </div>
          ...
          <TabsList className="bg-transparent h-auto p-0 gap-1 flex justify-start min-w-max">
            {[
              ...
              <TabsTrigger 
                key={tab.id}
                value={tab.id} 
                className="rounded-xl data-[state=active]:bg-muted data-[state=active]:text-foreground px-6 py-2.5 font-bold transition-all shadow-none border-none whitespace-nowrap uppercase text-[10px] tracking-widest"
              >
                {tab.label}
              </TabsTrigger>
            ))}
        ...
        <TabsContent value="dashboard" className="space-y-8 m-0 animate-in fade-in duration-500">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-6">
            <Card className="rounded-[2rem] border-none shadow-sm bg-card p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Managed Titles</span>
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-card p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Scholarly Audits</span>
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-card p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Digital Payouts</span>
          ...
          <Card className="rounded-[2rem] border-none shadow-sm bg-card">
            <CardHeader className="p-8 flex flex-row items-center justify-between">
              <div className="space-y-1">
                <CardTitle className="text-xl font-black text-foreground">Content Integrity Log</CardTitle>
                <p className="text-sm text-muted-foreground font-medium">Verify authentic source metadata for new digital course releases.</p>
              </div>
              <Button size="sm" className="bg-zinc-900 hover:bg-zinc-800 rounded-xl font-black text-xs h-10 px-6 text-white group shadow-lg shadow-slate-200">
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Publisher</TableHead>
                    <TableHead className="h-14 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Title</TableHead>
                    <TableHead className="h-14 font-black uppercase text-[10px] tracking-widest text-muted-foreground text-center">Format</TableHead>
                ...
                <TableBody>
                  {[
                    ...
                    <TableRow key={i} className="border-border hover:bg-muted/50 transition-colors">
                      ...
                      <TableCell className="font-bold text-muted-foreground text-xs italic">{item.title}</TableCell>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-10">
            <div className="space-y-10">
              <div className="space-y-2 border-b pb-6">
                <h3 className="text-3xl font-black text-foreground uppercase tracking-tight">Content Distribution Standards</h3>
                <p className="text-muted-foreground font-medium text-sm sm:text-lg italic">Global mandatory compliance criteria for media partners.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-5 bg-muted rounded-2xl border border-transparent hover:border-border transition-all cursor-pointer group shadow-sm">
                ...
                <div className="bg-zinc-900 rounded-[2.5rem] p-10 text-white space-y-6 relative overflow-hidden flex flex-col justify-between">
                  <Library className="absolute -top-4 -right-4 h-32 w-32 opacity-10 text-muted-foreground" />
                  <div className="space-y-2 relative z-10">
                    <h4 className="text-xl font-black text-muted-foreground uppercase tracking-tighter">Scholarly Oversight</h4>
                    <p className="text-muted-foreground text-sm">Automated reminders for content re-vetting cycles.</p>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-8">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-8 space-y-6 flex flex-col justify-between">
                <div className="space-y-2">
                  <h3 className="text-xl font-black text-foreground">Reader Tiers</h3>
                  <p className="text-sm text-muted-foreground font-medium italic">Manage how readers earn Hub Coins for buying authentic literature.</p>
                </div>
                <div className="space-y-4">
                  {[
        ...
        {["all", "verification", "reviews", "offers", "certificates", "categories", "billing"].map((tab) => (
          <TabsContent key={tab} value={tab} className="animate-in fade-in duration-500 m-0">
            <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-20 text-center space-y-6">
              ...
              <Button variant="outline" className="rounded-xl border-2 font-bold px-8 h-12 border-border text-muted-foreground hover:bg-muted">Refresh Catalog Data</Button>
            </Card>
          </TabsContent>
        ))}
      </Tabs>
