...
export default function SuperAdminTravelManagement() {
  ...
  const MOCK_AGENCIES = [
    { id: "TRV-001", name: "Saffron Travels", city: "London, UK", class: "Category A", status: "Verified", rating: 4.9, bookings: 1240 },
    { id: "TRV-002", name: "Al-Huda Pilgrimages", city: "Dubai, UAE", class: "Category A", status: "Verified", rating: 4.8, bookings: 850 },
    { id: "TRV-003", name: "Nomad Modest Tours", city: "Istanbul, TR", class: "Category B", status: "Audit Needed", rating: 4.7, bookings: 2100 },
    { id: "TRV-004", name: "Sky Halal Holidays", city: "New York, USA", class: "Category C", status: "Verified", rating: 4.5, bookings: 450 },
  ];

  return (
    <div className="container mx-auto p-6 space-y-8 max-w-7xl pb-24">
      <div className="space-y-1">
        <h1 className="text-3xl font-black font-headline text-slate-900 uppercase tracking-tighter">Travel & Tourism</h1>
        <p className="text-muted-foreground font-medium text-lg italic">Manage agencies, pilgrim partners, and Shariah-compliant itineraries.</p>
      </div>
        ...
        <TabsContent value="dashboard" className="space-y-8 m-0 animate-in fade-in duration-500">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Active Agencies</span>
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Vetted Pilgrim Partners</span>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase">Hajj & Umrah Vetted</p>
              </div>
            </Card>

            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8 group hover:shadow-md transition-all">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Total Bookings (MTD)</span>
                <div className="h-10 w-10 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600">
                  <Globe className="h-5 w-5" />
                </div>
              </div>
              <div className="space-y-1">
                <p className="text-4xl font-black text-slate-900">2.4k</p>
                <p className="text-[10px] font-bold text-slate-400 uppercase">Active Itineraries</p>
              </div>
            </Card>
          </div>
          ...
          <Card className="rounded-[2rem] border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="p-8 flex flex-row items-center justify-between">
              <div className="space-y-1">
                <CardTitle className="text-xl font-black text-slate-900">Pilgrimage Audit Queue</CardTitle>
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black uppercase text-[10px] tracking-widest text-slate-400">Agency</TableHead>
                    <TableHead className="h-14 font-black uppercase text-[10px] tracking-widest text-slate-400">License Class</TableHead>
                ...
                <TableBody>
                  {[
                    ...
                    <TableRow key={i} className="border-slate-100 hover:bg-slate-50/50 transition-colors">
                      ...
                      <TableCell className="font-bold text-slate-500 text-xs italic">{item.class}</TableCell>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="p-8 border-b space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative w-full md:w-96">
                  ...
                  <Input placeholder="Search agencies by name, ID or city..." className="pl-9 h-12 rounded-2xl bg-slate-50 border-none font-medium" />
                </div>
            ...
            <CardContent className="p-0">
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Agency Identity</TableHead>
                    ...
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400 text-center">Total Bookings</TableHead>
                ...
                <TableBody>
                  {MOCK_AGENCIES.map((agency) => (
                    <TableRow key={agency.id} className="border-slate-100 hover:bg-slate-50/50 transition-colors group">
                      <TableCell className="px-8 py-5">
                        <p className="font-black text-slate-800 text-base">{agency.name}</p>
                        <p className="text-[10px] font-bold text-primary uppercase tracking-tighter">{agency.id} • {agency.class}</p>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-1.5 font-black text-sm text-amber-500">
                          <Star className="h-3.5 w-3.5 fill-current border-none" /> {agency.rating}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-right px-8">
                        <Badge variant="outline" className={
                          agency.status === 'Verified' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 font-black text-[9px] px-3' : 'bg-amber-50 text-amber-600 border-amber-200 font-black text-[9px] px-3'
                        }>
                          {agency.status}
                        </Badge>
                      </TableCell>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-10">
            <div className="space-y-10">
              <div className="space-y-2 border-b pb-6">
                <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Pilgrim Charter Standards</h3>
                <p className="text-muted-foreground font-medium text-lg italic">Global mandatory compliance criteria for travel partners.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white space-y-6 relative overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="space-y-2 relative z-10">
                    ...
                    <p className="text-slate-400 text-sm">Automated reminders for agency package re-verifications.</p>
                  </div>
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-none">
                    ...
                    <TableHead className="h-14 font-black uppercase text-[10px] tracking-widest text-slate-400">Agency & Content</TableHead>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <button className="rounded-[2.5rem] border-4 border-dashed border-slate-100 bg-slate-50/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-white hover:border-primary/20 transition-all cursor-pointer group min-h-[300px]">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-slate-400 font-medium">Create seasonal or group package deals</p>
              </div>
            </button>
          </div>
        </TabsContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-8">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 space-y-6 flex flex-col justify-between">
                <div className="space-y-2">
                  <h3 className="text-xl font-black text-slate-900">Agency Tiers</h3>
        ...
        <TabsContent value="categories" className="animate-in fade-in duration-500 m-0">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="p-8 border-b flex flex-row items-center justify-between">
              <div className="space-y-1">
                <CardTitle className="text-xl font-black text-slate-900">Tour Taxonomy</CardTitle>
                ...
                <TableBody>
                  {[
                    ...
                    <TableRow key={i} className="border-slate-100 hover:bg-slate-50/50 transition-colors group">
                      ...
                      <TableCell className="font-bold text-slate-500 text-xs">{cat.count} agencies</TableCell>
                      <TableCell className="text-center">
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-10 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="p-8 bg-slate-50 rounded-[2rem] space-y-2 border shadow-inner">
                    ...
                    <p className="text-3xl font-black text-slate-900">7.5% <span className="text-sm font-bold text-slate-400 italic">flat</span></p>
                    <p className="text-xs font-bold text-emerald-600">On all digital package bookings</p>
                  </div>
                  <div className="p-8 bg-slate-50 rounded-[2rem] space-y-2 border shadow-inner">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Net GMV (MTD)</p>
                    <p className="text-3xl font-black text-slate-900">₹12.4M</p>
                    <p className="text-xs font-bold text-slate-400 uppercase">Travel vertical gross volume</p>
                  </div>
                </div>
                <div className="space-y-4">
