      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="lg:col-span-2 rounded-[2.5rem] border-none shadow-sm bg-card">
              <CardHeader className="p-8 border-b flex flex-row items-center justify-between bg-muted/30">
                ...
                <Button variant="outline" size="sm" className="rounded-xl font-black text-xs h-9 bg-card">Full Analytics</Button>
              </CardHeader>
              <CardContent className="p-8 h-[300px] flex items-center justify-center text-muted-foreground italic font-medium">
                [High-Fidelity Interaction Chart: Engagement vs Consistency]
              </CardContent>
            </Card>

            <div className="space-y-6">
              <h3 className="text-lg font-black text-foreground px-2 uppercase tracking-tighter">Top Earners (MTD)</h3>
              <div className="space-y-4">
                {MOCK_CREATORS.slice(0, 3).map((c, i) => (
                  <Card key={i} className="border-none shadow-sm rounded-2xl p-4 bg-card hover:bg-zinc-800 hover:text-white transition-all group cursor-pointer">
                    <div className="flex items-center gap-4">
                      <div className="h-10 w-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 group-hover:bg-card/10 group-hover:text-blue-400 font-black text-xs shadow-inner">
                        #{i + 1}
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-black tracking-tight">{c.name}</p>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase">{c.username}</p>
                      </div>
              ...
              <Button variant="outline" className="w-full rounded-xl h-12 font-black border-2 text-[10px] uppercase tracking-widest bg-card">Full Revenue Report</Button>
            </div>
          </div>
        </TabsContent>
            ...
            <CardHeader className="p-8 border-b space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative w-full md:w-96">
                  ...
                  <Input placeholder="Search name, username, city or ID..." className="pl-9 h-12 rounded-2xl bg-muted border-none font-medium" />
                </div>
                <div className="flex items-center gap-3">
                  <Select defaultValue="all">
                    <SelectTrigger className="w-32 h-12 rounded-xl bg-muted border-none font-bold text-xs uppercase">
                  ...
                  <Button variant="outline" className="rounded-xl h-12 gap-2 border-2 font-bold px-6 bg-card"><Filter className="h-4 w-4" /> Filters</Button>
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black text-[10px] uppercase tracking-widest text-muted-foreground">Creator Identity</TableHead>
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-muted-foreground">Reach / Stats</TableHead>
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-center">Int. Score</TableHead>
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-muted-foreground text-center">Tier</TableHead>
                    <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-muted-foreground">Status</TableHead>
                    <TableHead className="text-right px-8 h-14 font-black text-[10px] uppercase tracking-widest text-muted-foreground">Control</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {MOCK_CREATORS.map((crt) => (
                    <TableRow key={crt.id} className="border-border hover:bg-muted/50 transition-colors group">
                      <TableCell className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-black text-foreground text-base leading-tight">{crt.name}</p>
                            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-tighter">{crt.username} • {crt.category}</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <p className="font-black text-foreground text-sm">{crt.reach}</p>
                      ...
                      <TableCell className="text-center">
                        <div className="inline-flex flex-col items-center">
                          ...
                          <Progress value={crt.score} className="h-1 w-12 bg-muted" />
                        </div>
                      </TableCell>
                      <TableCell className="text-center">
                        <Badge className={cn(
                          "rounded-full px-3 text-[9px] font-black uppercase border-none",
                          crt.tier === 'Elite' ? 'bg-zinc-900 text-amber-400' : 
                          crt.tier === 'Gold' ? 'bg-amber-100 text-amber-700' : 'bg-muted text-muted-foreground'
                        )}>
                          {crt.tier}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-right px-8">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                          <Button size="icon" variant="ghost" className="rounded-xl hover:bg-card hover:shadow-sm"><Edit2 className="h-4 w-4 text-muted-foreground" /></Button>
                          <Button size="icon" variant="ghost" className="rounded-xl hover:bg-card hover:shadow-sm"><ShieldAlert className="h-4 w-4 text-muted-foreground" /></Button>
                          <Button size="icon" variant="ghost" className="rounded-xl hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4 text-muted-foreground" /></Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <TableRow key={i} className="border-border hover:bg-muted/50 transition-colors">
                          <TableCell className="px-8 py-6">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className="font-black text-foreground text-base">{post.creator}</p>
                                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{post.date} • {post.type}</p>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                            <div className="space-y-1">
                              <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Flag Category</p>
                              <Badge variant="outline" className={cn(
                          ...
                          <TableCell className="text-right px-8">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <Button size="icon" variant="ghost" className="rounded-xl bg-muted hover:bg-card shadow-none"><Eye className="h-5 w-5" /></Button>
                            </div>
                          </TableCell>
                        </TableRow>
                      ))}
            ...
            <div className="lg:col-span-4 space-y-6">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-zinc-900 text-white p-8 space-y-6 relative overflow-hidden">
                ...
                <div className="flex items-center justify-between p-4 bg-card/5 rounded-2xl border border-white/10 relative z-10">
                  <span className="text-xs font-bold text-muted-foreground">System Accuracy</span>
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-8 space-y-6">
                <h3 className="text-xl font-black text-foreground">Community Reports</h3>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-4 bg-muted rounded-2xl border border-transparent hover:border-rose-200 transition-all cursor-pointer group">
                      <span className="text-sm font-bold text-foreground">{r.label}</span>
                      <Badge className="bg-card text-rose-600 font-black text-[10px]">{r.count}</Badge>
                    </div>
                  ))}
                </div>
              </Card>
        ...
        <TabsContent value="campaigns" className="animate-in fade-in duration-500 m-0 space-y-8">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between px-2">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" className="rounded-xl h-12 px-6 font-bold border-2 bg-card">Campaign Analytics</Button>
            ...
            {[
              ...
              <Card key={i} className="rounded-[2.5rem] border-none shadow-sm bg-card flex flex-col group hover:shadow-xl transition-all duration-500 border-2 border-transparent hover:border-blue-100">
                <CardHeader className="p-8 pb-4">
                  ...
                  <h3 className="text-2xl font-black text-foreground leading-tight">{camp.title}</h3>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mt-1">Brand: {camp.brand}</p>
                </CardHeader>
                <CardContent className="p-8 pt-0 space-y-6">
                  <div className="flex justify-between items-end border-t border-border pt-6">
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Budget (Credits)</p>
                      <p className="text-xl font-black text-foreground">{camp.budget}</p>
                    </div>
                    <div className="text-right space-y-1">
                      <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Creators</p>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                      ...
                      <span className="text-foreground">{camp.completion}%</span>
                    </div>
                    <Progress value={camp.completion} className="h-1.5 bg-muted" />
                  </div>
                </CardContent>
                <CardFooter className="p-8 pt-0 mt-auto grid grid-cols-2 gap-3">
                  ...
                  <Button className="bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl h-11 font-black text-[10px] uppercase tracking-widest shadow-xl">Manage Leads</Button>
                </CardFooter>
              </Card>
            ))}
          </div>
        ...
        <TabsContent value="finance" className="animate-in fade-in duration-500 m-0 space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-8">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-10 space-y-10 relative overflow-hidden">
                <div className="flex justify-between items-center relative z-10">
                  <div className="space-y-1">
                    <h3 className="text-3xl font-black text-foreground uppercase tracking-tighter">Settlement Ledger</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                  <div className="p-8 bg-muted rounded-[2rem] border shadow-inner text-center space-y-2">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest leading-none mb-1">Total Creator Earnings</p>
                    <p className="text-5xl font-black text-foreground tracking-tighter">₹18.4M</p>
                    <p className="text-xs font-bold text-emerald-600 uppercase tracking-tighter">+12.4% vs prev. month</p>
                  </div>
                  <div className="p-8 bg-muted rounded-[2rem] border shadow-inner text-center space-y-2">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest leading-none mb-1">Platform Commission</p>
                    <p className="text-5xl font-black text-blue-600 tracking-tighter">₹1.8M</p>
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-tighter">10% Platform Flat Fee</p>
                  </div>
                </div>

                <div className="space-y-6">
                  <h4 className="text-sm font-black uppercase text-muted-foreground tracking-widest px-2">Pending Disursements</h4>
                  <div className="divide-y border rounded-[2rem] overflow-hidden bg-card">
                    {[
                      ...
                      <div key={i} className="p-6 bg-card flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-muted transition-colors group">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 bg-muted rounded-xl flex items-center justify-center text-muted-foreground font-black text-xs">
                            {set.creator[1].toUpperCase()}
                          </div>
                          <div>
                            <p className="text-sm font-black text-foreground">{set.creator}</p>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{set.id} • {set.date}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-8">
                          <span className="text-xl font-black text-foreground">{set.amount}</span>
                          <Badge className={cn(
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-zinc-900 text-white p-8 space-y-8 relative overflow-hidden flex flex-col justify-between h-full">
                ...
                <div className="relative z-10 space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-bold text-muted-foreground uppercase leading-relaxed max-w-[200px]">
                ...
                <div className="space-y-4 relative z-10 pt-8">
                  ...
                  <Button variant="outline" className="w-full border-white/20 text-white hover:bg-card/10 rounded-2xl h-14 font-black uppercase text-xs tracking-widest">Adjust Payout Rules</Button>
                </div>
              </Card>
            </div>
          </div>
        ...
        <TabsContent value="economy" className="animate-in fade-in duration-500 m-0 space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="lg:col-span-2 rounded-[2.5rem] border-none shadow-sm bg-card p-10 space-y-10">
              <div className="flex justify-between items-center">
                <div className="space-y-1">
                  <h3 className="text-3xl font-black text-foreground tracking-tighter uppercase">Economy Governance</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-8">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-[10px] uppercase text-muted-foreground tracking-widest">Earn Rate (per Like)</Label>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-[10px] uppercase text-muted-foreground tracking-widest">Conversion Rate (Redeem)</Label>
                ...
                <div className="space-y-8">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-[10px] uppercase text-muted-foreground tracking-widest">Daily User Limit</Label>
                  ...
                  <div className="p-6 bg-muted rounded-2xl space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-black text-foreground uppercase tracking-tighter">Freeze Economy</span>
                      <Switch />
                    </div>
                    <p className="text-[10px] text-muted-foreground font-medium italic">Emergency halt for all coin issuances and redemptions across the platform.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-border flex justify-end gap-4">
                ...
                <Button className="bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl h-12 px-10 font-black uppercase text-[10px] tracking-widest shadow-xl">Apply Changes</Button>
              </div>
            </Card>

            <div className="space-y-8">
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-8 space-y-6">
                <h3 className="text-xl font-black text-foreground">Inflation Monitor</h3>
                <div className="space-y-6">
                  {[
              ...
              <Card className="rounded-[2.5rem] border-none shadow-sm bg-amber-50 p-8 space-y-4 border-2 border-amber-100">
                ...
                <Button variant="outline" size="sm" className="w-full rounded-xl border-amber-200 text-amber-700 bg-card font-black text-[9px] uppercase tracking-widest h-9">Investigate Abuse</Button>
              </Card>
            </div>
          </div>
        </TabsContent>
            ...
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-8">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-xs uppercase text-muted-foreground tracking-widest">Engagement Weight</Label>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-xs uppercase text-muted-foreground tracking-widest">Content Quality (AI Score)</Label>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-xs uppercase text-muted-foreground tracking-widest">Consistency Factor</Label>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <Label className="font-black text-xs uppercase text-muted-foreground tracking-widest">Campaign Success ROI</Label>
                ...
                <div className="bg-zinc-900 rounded-[2.5rem] p-10 text-white space-y-8 relative overflow-hidden flex flex-col justify-between">
                  ...
                  <div className="space-y-4 relative z-10">
                    <Button variant="secondary" className="w-full rounded-2xl font-black text-xs h-14 uppercase tracking-widest shadow-xl bg-card text-foreground">Configure Event Boosts</Button>
                    <p className="text-[10px] text-center text-muted-foreground font-bold uppercase tracking-widest">Active Manual Boosts: 0</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {["safety", "growth"].map((tab) => (
          <TabsContent key={tab} value={tab} className="animate-in fade-in duration-500 m-0">
            <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-24 text-center space-y-8">
              ...
              <Button variant="outline" className="rounded-2xl border-2 font-black px-12 h-14 uppercase text-xs tracking-[0.2em] hover:bg-muted">Sync System Data</Button>
            </Card>
          </TabsContent>
        ))}
      </Tabs>
