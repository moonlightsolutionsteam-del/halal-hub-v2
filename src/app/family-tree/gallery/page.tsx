    ...
    <div className="container mx-auto p-6 space-y-10 max-w-6xl pb-24 text-slate-900">
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {ALBUMS.map((album) => (
            <Card key={album.id} className="group rounded-[2rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl transition-all duration-500 cursor-pointer">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between px-2">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <Input placeholder="Search moments..." className="pl-9 h-11 rounded-2xl bg-white border-none shadow-sm font-medium" />
            </div>
          ...
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div key={i} className="relative aspect-[4/5] rounded-[2rem] overflow-hidden group shadow-md hover:shadow-2xl transition-all duration-700 cursor-pointer border-2 border-transparent hover:border-emerald-100">
              ...
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button size="icon" variant="secondary" className="h-8 w-8 rounded-lg bg-white/20 backdrop-blur-md border-none text-white hover:bg-white/40"><Heart className="h-4 w-4" /></Button>
              </div>
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
                ...
                <div className="flex gap-1">
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-white hover:bg-white/20"><Download className="h-3.5 w-3.5" /></Button>
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-white hover:bg-white/20"><MoreVertical className="h-3.5 w-3.5" /></Button>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* AI Memory Card */}
      <Card className="rounded-[3rem] border-none bg-slate-900 text-white p-12 relative overflow-hidden flex flex-col md:flex-row items-center gap-12">
        <Sparkles className="absolute -top-4 -right-4 h-48 w-48 opacity-10 text-emerald-400" />
        <div className="h-24 w-24 rounded-[2rem] bg-white/10 flex items-center justify-center text-emerald-400 border border-white/10 shadow-2xl shrink-0 group hover:rotate-12 transition-transform">
