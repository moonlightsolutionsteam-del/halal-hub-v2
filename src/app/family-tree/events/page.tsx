  ...
  return (
    <div className="container mx-auto p-6 space-y-8 max-w-5xl pb-24">
      ...
      <div className="grid grid-cols-1 gap-6">
        {events.map((event) => (
          <Card key={event.id} className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white group hover:shadow-xl transition-all border-2 border-transparent hover:border-emerald-100">
            <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-10">
              <div className="flex items-center gap-8 flex-1">
                <div className="h-20 w-20 rounded-[2rem] bg-slate-50 flex items-center justify-center text-emerald-600 group-hover:scale-110 transition-transform shadow-inner shrink-0">
                ...
                <div className="space-y-2 min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> {event.date}</span>
                  </div>
                  <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-tight truncate">{event.title}</h3>
              ...
              <div className="flex items-center gap-12 w-full md:w-auto border-t md:border-t-0 pt-6 md:pt-0">
                <div className="text-center md:text-right">
                  <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Attending</p>
                  <div className="flex items-center gap-2 justify-center md:justify-end">
                    <Users className="h-4 w-4 text-emerald-600" />
                    <span className="text-xl font-black text-slate-900">{event.attending}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button size="icon" variant="ghost" className="rounded-xl h-12 w-12 border">
                    {event.reminder ? <Bell className="h-5 w-5 text-emerald-600 fill-current" /> : <BellOff className="h-5 w-5 text-slate-300" />}
                  </Button>
      ...
      <Card className="rounded-[2.5rem] border-none bg-emerald-50 p-10 flex flex-col md:flex-row items-center justify-between gap-8 border-2 border-emerald-100">
        ...
        <Link href="/family-tree/discovery">
          <Button variant="secondary" className="rounded-2xl h-14 px-10 font-black uppercase text-xs tracking-widest bg-white text-emerald-600 shadow-sm border-none">Browse Suggestions <Sparkles className="ml-2 h-4 w-4" /></Button>
        </Link>
      </Card>
    </div>
  );
