    ...
    <div className="container mx-auto p-6 space-y-8 max-w-5xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <h1 className="text-3xl font-black font-headline text-slate-900">Family Board</h1>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[2rem] shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <Input placeholder="Search board..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none font-medium" />
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {boardItems.filter(item => item.status.toLowerCase() === activeTab).map((item) => (
          <Card key={item.id} className="rounded-[2rem] border-none shadow-sm overflow-hidden bg-white group hover:shadow-md transition-all border-2 border-transparent hover:border-emerald-100">
            <div className="p-8 flex flex-col md:flex-row gap-8 items-start">
              <div className="flex-1 space-y-4">
                ...
                <div>
                  <h3 className={cn("text-2xl font-black text-slate-900 leading-tight", item.status === 'Completed' && "line-through opacity-40")}>{item.title}</h3>
                  <p className="text-slate-500 font-medium mt-1 leading-relaxed italic">"{item.desc}"</p>
                </div>
      ...
      <Card className="rounded-[2.5rem] border-none bg-slate-900 text-white p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 font-medium">You can create board items, assign members, and manage events for the whole family.</p>
          </div>
        </div>
      </Card>
    </div>
