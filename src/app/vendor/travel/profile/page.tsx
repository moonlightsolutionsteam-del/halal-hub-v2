    ...
    <div className="container mx-auto p-6 space-y-8 max-w-7xl pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-black font-headline tracking-tight text-slate-900">Travel Agency Profile</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <Label className="font-bold text-xs uppercase tracking-widest text-slate-500">Agency Name</Label>
                  <Input placeholder="e.g., Saffron Travels" className="h-12 rounded-2xl bg-slate-50 border-none font-bold" />
                </div>
                <div className="space-y-2">
                  <Label className="font-bold text-xs uppercase tracking-widest text-slate-500">Contact Email</Label>
                  <Input placeholder="info@travelagency.com" className="h-12 rounded-2xl bg-slate-50 border-none font-bold" />
                </div>
                <div className="md:col-span-2 space-y-2">
                  <Label className="font-bold text-xs uppercase tracking-widest text-slate-500">Agency Bio</Label>
                  <Textarea placeholder="Describe your experience, network, and specializations..." className="min-h-[120px] rounded-2xl bg-slate-50 border-none p-4 font-medium" />
                </div>
              </div>
            </Card>
          </section>
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {TRUST_BADGES.map((badge) => (
                  <div key={badge} className="flex items-center space-x-3 p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-amber-200 transition-all">
                    <Checkbox id={`b-${badge}`} />
                    <label htmlFor={`b-${badge}`} className="text-sm font-bold text-slate-700">{badge}</label>
                  </div>
                ))}
              </div>
            </Card>
        ...
        <TabsContent value="packages" className="animate-in fade-in duration-500 space-y-10">
            ...
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {PACKAGE_TYPES.map((pkg) => (
                  <div key={pkg} className="flex items-center space-x-3 p-4 bg-slate-50 rounded-2xl">
                    <Checkbox id={`p-${pkg}`} />
                    <label htmlFor={`p-${pkg}`} className="text-sm font-bold text-slate-700">{pkg}</label>
                  </div>
                ))}
              </div>
            </Card>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-xl bg-slate-900 text-white p-10 space-y-8">
            ...
            <p className="text-sm text-slate-400 font-medium">I confirm that all Hajj/Umrah packages are provided through licensed operators and that halal board is guaranteed as stated in itineraries.</p>
