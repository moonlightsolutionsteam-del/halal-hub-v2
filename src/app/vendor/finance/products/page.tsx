    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline text-slate-900">Financial Products</h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[2rem] shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <Input placeholder="Search products by name or SKU..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none font-medium" />
        </div>
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="px-4 py-2 rounded-full cursor-pointer border-slate-200">Investments</Badge>
          <Badge variant="outline" className="px-4 py-2 rounded-full cursor-pointer border-slate-200">Banking</Badge>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
        {products.map((product) => (
          <Card key={product.id} className="group rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl transition-all duration-500 border-2 border-transparent hover:border-indigo-100">
            <div className="p-8 flex flex-col sm:flex-row gap-8">
              <div className="relative h-32 w-full sm:w-32 rounded-[2rem] bg-indigo-50 flex items-center justify-center shrink-0 shadow-inner group-hover:scale-105 transition-transform duration-700">
                ...
                <Badge className="absolute -top-2 -right-2 bg-white text-indigo-600 font-black border-2 border-indigo-50 text-[8px] h-6 px-2 flex items-center shadow-lg">NEW</Badge>
              </div>
              <div className="flex-1 space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">SKU: {product.code}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between border-t border-slate-50 pt-4">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`h-2 w-2 rounded-full ${product.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                    <span className="text-[10px] font-black text-slate-400 uppercase">{product.status}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <button className="rounded-[2.5rem] border-4 border-dashed border-slate-100 bg-slate-50/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-white hover:border-indigo-200 transition-all cursor-pointer group min-h-[250px]">
          ...
          <div className="space-y-1">
            <p className="font-black text-xl text-slate-900">Add New Offering</p>
            <p className="text-sm text-slate-400 font-medium px-4">Register a new Shariah-compliant financial instrument</p>
          </div>
        </button>
      </div>
    </div>
