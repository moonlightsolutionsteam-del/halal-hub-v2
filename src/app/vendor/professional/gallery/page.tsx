    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8">
      <div className="flex items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm font-bold text-muted-foreground">Photos of your practice and facilities.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {photos.map((src, i) => (
          <Card key={i} className="rounded-[1.5rem] border-none shadow-soft overflow-hidden aspect-square relative group">
            <Image src={src} alt={`Practice photo ${i + 1}`} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
          </Card>
        ))}
      </div>
    </div>
