...
export default function HotelGalleryPage() {
  ...
  if (!businessId) {
    return <div className="p-8 text-center text-muted-foreground">Register your property to manage a gallery.</div>
  }
  ...
  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-6">
        {images.map((img, i) => (
          <Card key={img} className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden group hover:shadow-xl transition-all border-2 border-transparent hover:border-sky-100">
            <div className="relative aspect-video">
              <Image src={img} alt={`Property photo ${i + 1}`} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
              {i === 0 && (
                <Badge className="absolute top-4 left-4 bg-sky-600 text-white border-none px-3 font-black text-[9px] uppercase tracking-widest shadow-xl">COVER PHOTO</Badge>
              )}
            ...
            {i !== 0 && (
              <div className="p-6 flex items-center justify-end">
                <Button variant="ghost" size="sm" onClick={() => setPrimary(img)} className="text-[10px] font-black uppercase text-sky-600 p-0 h-auto hover:bg-transparent hover:underline">Set as cover</Button>
              </div>
            )}
          </Card>
        ))}

        <label className="rounded-[2.5rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-sky-200 transition-all cursor-pointer group min-h-[250px]">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground font-medium">Add room or amenity photos</p>
          </div>
