    ...
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl sm:text-3xl font-black font-headline text-foreground">Rooms & Suites</h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card p-4 rounded-[2.5rem] shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <Input placeholder="Search room names, types..." className="pl-9 h-11 rounded-2xl bg-muted border-none font-medium" />
        </div>
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="px-4 py-2 rounded-full cursor-pointer hover:bg-muted border-border">Suites</Badge>
          <Badge variant="outline" className="px-4 py-2 rounded-full cursor-pointer hover:bg-muted border-border">Available</Badge>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
        {rooms.map((room) => (
          <Card key={room.id} className="group rounded-[3rem] border-none shadow-sm overflow-hidden bg-card hover:shadow-xl transition-all duration-500 border-2 border-transparent hover:border-sky-100">
            <div className="p-8 flex gap-8">
              <div className="relative h-32 w-32 rounded-[2rem] overflow-hidden shrink-0 shadow-lg">
                <Image src={`https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=400/400`} alt={room.name} fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
              </div>
              <div className="flex-1 space-y-4">
                <div className="space-y-1">
                  ...
                  <h3 className="text-2xl font-black text-foreground tracking-tight leading-tight">{room.name}</h3>
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Inventory: {room.inventory}</p>
                </div>
                <div className="flex items-center justify-between border-t border-border pt-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-muted-foreground uppercase">{room.status}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <button className="rounded-[3rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-sky-200 transition-all cursor-pointer group">
          ...
          <div className="space-y-1">
            <p className="font-black text-xl text-foreground">Add New Inventory</p>
            <p className="text-sm text-muted-foreground font-medium px-4">Register a new room category or individual suite</p>
          </div>
        </button>
      </div>
    </div>
