    ...
    <div className="container mx-auto p-6 space-y-8 max-w-7xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline text-slate-900">Catering Logistics</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden h-[450px] relative">
            ...
            <div className="relative z-10 h-full flex flex-col items-center justify-center gap-4">
              <div className="h-16 w-16 bg-white rounded-full flex items-center justify-center shadow-2xl border-4 border-blue-600">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-50">
                {deployments.map((del) => (
                  <div key={del.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-slate-50/50 transition-colors group">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="font-black text-slate-900 text-sm">{del.id}</p>
                        <p className="font-bold text-slate-700">{del.event}</p>
                    ...
                    <div className="flex items-center gap-10">
                      <div className="text-right">
                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Thermal Stat</p>
                        <Badge className={
                      ...
                      <div className="text-right">
                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Arrival</p>
                        <p className="text-sm font-black text-blue-600">{del.time}</p>
                      </div>
                      <Button size="icon" variant="ghost" className="rounded-xl hover:bg-white hover:shadow-md transition-all"><ArrowUpRight className="h-5 w-5 text-slate-300" /></Button>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group cursor-pointer hover:shadow-md transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Cap: {v.cap}</p>
                    </div>
                  </div>
                  <Badge className={v.status === 'Active' ? 'bg-emerald-50 text-emerald-600 border-none' : 'bg-slate-100 text-slate-400 border-none'}>
                    {v.status}
                  </Badge>
                </div>
              ))}
