...
export default function CateringReviewsPage() {
  const reviews = [
    { id: 1, user: "Sarah Ahmed", rating: 5, date: "2 hours ago", comment: "The Mutton Biryani for our Nikah was spectacular! Every guest asked for the caterer's name. Professional staff and impeccable presentation.", response: "Thank you Sarah! It was an honor to serve your family." },
    { id: 2, user: "Omar Malik", rating: 4, date: "1 day ago", comment: "Great food quality. The setup was a little delayed due to traffic but the team worked fast to catch up. The Kabobs were amazing.", response: null },
    { id: 3, user: "Layla Khalil", rating: 5, date: "3 days ago", comment: "Elite standard of service. The live tandoor station was the highlight of our corporate event. Highly recommended.", response: "JazakAllah Layla, glad the team could deliver a great experience!" },
  ];
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-10 text-center space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Global Catering Score</p>
            </div>
              ...
              {[5, 4, 3, 2, 1].map((star) => (
                <div key={star} className="flex items-center gap-4">
                  ...
                  <div className="h-2 bg-slate-50 rounded-full flex-1 overflow-hidden shadow-inner">
                    <div className="h-full bg-blue-600 rounded-full" style={{ width: star === 5 ? '92%' : star === 4 ? '6%' : '2%' }} />
                  </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-slate-900 text-white p-8 space-y-6">
            <div className="flex items-center gap-3">
              <div className={`h-10 w-10 bg-white/10 rounded-xl flex items-center justify-center text-blue-600`}>
                <TrendingUp className="h-5 w-5" />
              </div>
              <h3 className="text-lg font-black tracking-tight">Trust Growth</h3>
            </div>
            <p className="text-sm text-slate-400 font-medium leading-relaxed">
              Your "Shariah Verified Logistics" badge has increased corporate inquiry volume by 25% this month.
            </p>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between px-2">
            <div className="relative w-full md:w-80">
              ...
              <Input placeholder="Search event reviews..." className="pl-9 h-11 rounded-2xl bg-white border-none shadow-sm font-medium" />
            </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {reviews.map((rev) => (
              <Card key={rev.id} className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden border-2 border-transparent hover:border-blue-100 transition-all group">
                <div className="p-8 space-y-6">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-12 w-12 border-2 border-slate-50 shadow-sm">
                        <AvatarImage src={`https://picsum.photos/seed/user-cat${rev.id}/100/100`} />
                  ...
                  {rev.response ? (
                    <div className="p-6 bg-slate-50 rounded-3xl border-l-4 border-blue-600 space-y-2">
                      <div className="flex items-center gap-2 text-xs font-black text-blue-600 uppercase tracking-widest">
                        <CheckCircle2 className="h-3.5 w-3.5" /> Catering Response
                      </div>
                      <p className="text-sm font-bold text-slate-700">{rev.response}</p>
                    </div>
                  ) : (
                    <div className="pt-4 flex gap-3">
                      <Button className="rounded-2xl h-11 px-6 font-black uppercase text-[10px] tracking-widest bg-blue-600 text-white shadow-lg shadow-blue-200">
