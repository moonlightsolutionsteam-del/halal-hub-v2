  ...
  return (
    <div className="p-8 space-y-8 bg-[#F8FBF9] min-h-screen">
      <div className="space-y-1">
        <h1 className="text-3xl font-black font-headline text-slate-900 uppercase tracking-tighter">Studio Control Room</h1>
        ...
        {kpis.map((kpi, i) => (
          <Card key={i} className="border-none shadow-sm rounded-3xl bg-white p-2 group hover:shadow-md transition-all">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <kpi.icon className={cn("h-4 w-4", kpi.variant === 'destructive' ? 'text-red-400' : 'text-slate-300 group-hover:text-primary transition-colors')} />
            </CardHeader>
            <CardContent>
              <div className={cn("text-3xl font-black", kpi.variant === 'destructive' ? 'text-red-600' : 'text-slate-800')}>
                {kpi.value}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <Card className="rounded-[2rem] border-none shadow-sm overflow-hidden bg-white">
            <CardHeader className="p-8 flex flex-row items-center justify-between bg-slate-50/30 border-b">
              <div className="space-y-1">
                <CardTitle className="text-xl font-black">Active Production Pipeline</CardTitle>
                <p className="text-sm font-medium text-slate-400">Content currently in editing or planning phases.</p>
              </div>
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="border-none">
                    <TableHead className="px-8 h-14 font-black uppercase text-[10px] tracking-widest text-slate-400">Project / ID</TableHead>
          ...
          <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8">
            ...
            <CardContent className="px-0 grid grid-cols-2 sm:grid-cols-3 gap-4">
              {quickActions.map((action, i) => (
                <button key={i} className="group flex flex-col items-center justify-center p-6 bg-slate-50/50 rounded-[2rem] hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-primary/10 shadow-inner">
                  ...
                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-tighter text-center leading-tight">{action.label}</span>
                </button>
              ))}
            </CardContent>
          </Card>
        ...
        <div className="space-y-8">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-slate-900 text-white p-10 overflow-hidden relative">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <Button variant="secondary" className="w-full rounded-2xl h-14 font-black text-xs uppercase tracking-widest shadow-2xl bg-white text-slate-900 hover:bg-slate-100">Browse Brand Deals</Button>
            </div>
          </Card>
        </div>
      </div>
