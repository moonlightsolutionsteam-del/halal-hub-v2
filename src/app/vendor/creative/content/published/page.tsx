    ...
    <div className="container mx-auto p-6 space-y-10 max-w-6xl pb-24 text-slate-900">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <h1 className="text-3xl font-black font-headline text-slate-900">Published Archive</h1>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[2rem] shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <Input placeholder="Search archive..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none font-medium" />
        </div>
        <div className="flex items-center gap-2">
          {["All", "Reels", "Podcasts", "Articles"].map(f => (
            <Badge key={f} variant="secondary" className="px-4 py-2 rounded-full cursor-pointer hover:bg-primary hover:text-white transition-all bg-slate-50 text-slate-500 font-black text-[10px] uppercase tracking-widest border-none">{f}</Badge>
          ))}
      ...
      <div className="grid grid-cols-1 gap-6">
        {PUBLISHED_CONTENT.map((content) => (
          <Card key={content.id} className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl transition-all duration-500 border-2 border-transparent hover:border-primary/10 group">
            <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-10">
              <div className="flex items-center gap-8 flex-1">
                ...
                <div className="space-y-2 min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-1.5"><Clock className="h-3 w-3" /> Published {content.date}</span>
                  </div>
                  <h3 className="text-xl font-black text-slate-900 truncate leading-tight group-hover:text-primary transition-colors">{content.title}</h3>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                <Button variant="outline" className="flex-1 sm:flex-none rounded-xl h-11 border-2 font-black text-xs uppercase tracking-widest bg-white hover:bg-slate-50">Analytics</Button>
                <Button variant="ghost" size="icon" className="rounded-xl h-11 w-11 bg-slate-50 text-slate-400 hover:text-primary transition-all"><Edit2 className="h-4 w-4" /></Button>
                <Button variant="ghost" size="icon" className="rounded-xl h-11 w-11 bg-slate-50 text-slate-400 hover:text-rose-600 transition-all"><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          </Card>
        ))}
