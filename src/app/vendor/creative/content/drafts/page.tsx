  ...
  return (
    <div className="container mx-auto p-6 space-y-10 max-w-5xl pb-24 text-slate-900">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <h1 className="text-3xl font-black font-headline text-slate-900">Drafts & Production</h1>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {DRAFTS.map((draft) => (
          <Card key={draft.id} className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden border-2 border-transparent hover:border-primary/10 transition-all group">
            <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-10">
              <div className="flex items-center gap-8 flex-1">
                <div className="h-16 w-16 rounded-[1.5rem] bg-slate-50 flex items-center justify-center text-slate-300 group-hover:text-primary transition-colors shadow-inner shrink-0">
                  <Clock className="h-8 w-8" />
                </div>
                <div className="space-y-2 min-w-0">
                  <div className="flex items-center gap-3">
                    <Badge variant="secondary" className="bg-slate-50 text-slate-400 border-none font-black text-[9px] uppercase px-3 h-6 flex items-center">{draft.category}</Badge>
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-1.5"><History className="h-3 w-3" /> Edited {draft.lastEdit}</span>
                  </div>
                  <h3 className="text-xl font-black text-slate-900 truncate tracking-tight">{draft.title}</h3>
                  <div className="flex items-center gap-4 pt-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black text-slate-400">{draft.completion} ready</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <Button className="rounded-xl h-11 px-8 font-black text-xs uppercase tracking-widest bg-slate-900 text-white shadow-xl hover:bg-primary transition-all">Resume Work</Button>
                <Button variant="ghost" size="icon" className="rounded-xl h-11 w-11 bg-slate-50 text-slate-400"><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          </Card>
        ))}
