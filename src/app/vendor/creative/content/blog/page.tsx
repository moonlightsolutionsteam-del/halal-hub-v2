          ...
          <Card key={art.id} className="rounded-[3rem] border-none shadow-sm overflow-hidden bg-card hover:shadow-xl transition-all duration-500 border-2 border-transparent hover:border-primary/10 group">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="p-8 flex-1 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge className={cn("border-none text-[9px] font-black uppercase px-3 h-6 flex items-center", art.status === 'Published' ? 'bg-emerald-50 text-emerald-600' : 'bg-muted text-muted-foreground')}>
                        {art.status}
                      </Badge>
                    </div>
                    <Button variant="ghost" size="icon" className="rounded-xl h-10 w-10 bg-muted opacity-0 group-hover:opacity-100 transition-opacity"><MoreVertical className="h-5 w-5 text-muted-foreground" /></Button>
                  </div>
                  <h3 className="text-2xl font-black text-foreground tracking-tight leading-tight group-hover:text-primary transition-colors">{art.title}</h3>
                ...
                <div className="pt-8 flex justify-end gap-3 border-t border-border mt-6">
                  <Button variant="outline" className="rounded-xl h-11 px-8 border-2 font-black text-xs uppercase tracking-widest bg-card">Edit Draft</Button>
                  <Button className="rounded-xl h-11 px-8 font-black text-xs uppercase tracking-widest bg-zinc-900 text-white shadow-xl">Promote Article</Button>
                </div>
              </div>
            </div>
          </Card>
