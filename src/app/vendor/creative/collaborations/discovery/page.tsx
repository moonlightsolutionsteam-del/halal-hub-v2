...
export default function DiscoveryHubPage() {
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-6 sm:space-y-10 max-w-7xl pb-24 text-foreground">
      ...
      {/* Filter & Search Bar */}
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card p-4 rounded-[2.5rem] border shadow-sm">
        <div className="relative w-full md:w-[450px]">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
          <Input placeholder="Search by campaign, requirements, or niche..." className="pl-12 h-14 rounded-2xl bg-muted border-none font-medium text-lg" />
        </div>
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full md:w-auto px-2">
          ...
          <Button variant="ghost" size="icon" className="h-14 w-14 rounded-2xl border bg-card shadow-sm shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="flex items-center justify-between px-4">
            <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Available Opportunities (<span className="text-primary">{OPEN_DEALS.length}</span>)</p>
              ...
              <Card key={deal.id} className="group rounded-[3rem] border-none shadow-sm overflow-hidden bg-card hover:shadow-2xl transition-all duration-700 flex flex-col h-full border-2 border-transparent hover:border-primary/10">
                ...
                <CardContent className="px-8 pb-8 flex-1 space-y-6">
                  <div className="grid grid-cols-2 gap-6 pt-2">
                    <div className="space-y-1">
                      <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Est. Budget</p>
                      <p className="text-lg font-black text-foreground">{deal.budget}</p>
                    </div>
                    <div className="text-right sm:text-left space-y-1">
                      <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Deliverables</p>
                      <p className="text-sm font-bold text-foreground truncate">{deal.requirements}</p>
                    </div>
                  </div>
                </CardContent>

                <CardFooter className="px-8 pb-8 pt-0 mt-auto">
                  <Button className="w-full bg-zinc-900 hover:bg-primary text-white rounded-[1.5rem] font-black text-sm uppercase tracking-widest h-16 shadow-2xl transition-all group-hover:scale-[1.02] gap-2">
        ...
        <aside className="lg:col-span-4 space-y-10">
          <Card className="rounded-[3rem] border-none shadow-sm bg-card p-10 space-y-10 sticky top-24 border border-border">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground font-medium">Top-tier partners currently looking for creative collaborations.</p>
            </div>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-5 bg-muted rounded-3xl border border-transparent hover:border-blue-100 transition-all cursor-pointer group">
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 rounded-2xl bg-card flex items-center justify-center text-muted-foreground font-black text-xs shadow-sm group-hover:scale-110 transition-transform">
                      {spot.name[0]}
                    </div>
                    <div>
                      <p className="text-sm font-black text-foreground blur-sm select-none">XXXXXXXXXXXX</p>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{spot.type}</p>
                    </div>
                  </div>
                  <Badge className="bg-card text-blue-600 font-black border-none text-[9px] h-6 px-2">{spot.score}% Trust</Badge>
                </div>
              ))}
            </div>

            <Card className="rounded-3xl border-none bg-zinc-900 text-white p-10 space-y-6 relative overflow-hidden shadow-2xl">
              <Zap className="absolute top-0 right-0 p-4 h-24 w-24 opacity-10 text-primary" />
              <div className="relative z-10 space-y-4 text-center">
                <div className="h-16 w-16 bg-card/10 rounded-2xl flex items-center justify-center mx-auto border border-white/10 shadow-3xl">
                ...
                <Button variant="outline" className="w-full border-white/20 text-white hover:bg-card/10 h-12 rounded-xl font-black text-[10px] uppercase tracking-widest">Read Charter</Button>
              </div>
            </Card>
          </Card>
        </aside>
