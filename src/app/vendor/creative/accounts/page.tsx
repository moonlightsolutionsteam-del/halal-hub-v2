...
const ACCOUNTS = [
  ...
  { id: 3, name: "Twitter / X", handle: "@hamza_deen", icon: Globe, status: "Connected", stats: "42k Follow", color: "text-foreground", bg: "bg-muted" },
];
  ...
  return (
    <div className="container mx-auto p-6 space-y-10 max-w-4xl pb-24 text-foreground">
      ...
      <div className="grid grid-cols-1 gap-6">
        {ACCOUNTS.map((acc) => (
          <Card key={acc.id} className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden border-2 border-transparent hover:border-primary/10 transition-all group">
            <div className="p-8 flex flex-col sm:flex-row items-center justify-between gap-10">
              <div className="flex items-center gap-8">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-base font-bold text-muted-foreground leading-none">{acc.handle}</p>
                  <p className="text-xs font-black text-muted-foreground uppercase tracking-widest pt-1">{acc.stats}</p>
                </div>
              </div>
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <Button variant="outline" className="flex-1 sm:flex-none rounded-xl h-12 px-8 font-black text-xs uppercase tracking-widest border-2 bg-card">Sync Data</Button>
                <Button variant="ghost" size="icon" className="rounded-xl h-12 w-12 bg-muted text-muted-foreground hover:bg-card hover:shadow-sm"><MoreVertical className="h-5 w-5" /></Button>
              </div>
            </div>
          </Card>
        ))}

        <Card className="rounded-[2.5rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-primary/20 transition-all cursor-pointer group">
          ...
          <div className="space-y-1">
            <p className="font-black text-xl text-foreground">Add Platform</p>
            <p className="text-sm text-muted-foreground font-medium px-4">Connect TikTok, Substack, or your own Podcast RSS</p>
          </div>
        </Card>
      </div>

      <div className="p-10 bg-zinc-900 text-white rounded-[3rem] relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl">
        ...
        <Button variant="outline" className="h-16 px-12 rounded-2xl border-2 border-white/20 text-white hover:bg-card/10 font-black uppercase text-sm tracking-widest relative z-10">Data Privacy Charter</Button>
      </div>
    </div>
  )
}
