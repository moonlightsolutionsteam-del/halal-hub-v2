      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              {activePrograms.map((program) => (
                <Card key={program.id} className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden border-2 border-transparent hover:border-emerald-100 transition-all group">
                  <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-10">
                    <div className="flex items-center gap-8">
                      <div className="h-16 w-16 rounded-3xl bg-slate-50 flex items-center justify-center text-emerald-600 shadow-inner group-hover:scale-110 transition-transform">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 space-y-6">
            <h3 className="text-xl font-black text-slate-900">Top Reward Targets</h3>
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-emerald-100 transition-all group cursor-pointer">
                  <span className="text-sm font-bold text-slate-700">{item.name}</span>
                  <Badge variant="secondary" className="bg-white text-slate-400 font-black text-[10px]">{item.count}</Badge>
                </div>
              ))}
            </div>
          </Card>
