...
export default function MarketingTransparencyPage() {
  const auditLogs = [
    { id: "GRO-AUD-101", topic: "Meat Counter Source Verification", status: "Pass", date: "Nov 01, 2024", score: "100%" },
    { id: "GRO-AUD-102", topic: "Bakery Ingredient Audit", status: "Pass", date: "Oct 28, 2024", score: "98%" },
    { id: "GRO-AUD-103", topic: "Store-wide Hygiene Standards", status: "Pass", date: "Oct 15, 2024", score: "100%" },
  ];
    ...
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium">Manage your supermarket's halal certifications, fresh produce logs, and audit history.</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button className="bg-emerald-600 hover:bg-emerald-700 rounded-full px-8 font-black shadow-lg shadow-emerald-200 h-12 text-white">
            <Award className="mr-2 h-4 w-4" /> Request Platform Audit
          </Button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-emerald-600 text-white p-10 text-center space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs font-bold uppercase tracking-widest opacity-80">Store Transparency Score</p>
          </div>
          <Badge className="bg-card text-emerald-600 font-black border-none uppercase text-[9px] px-4 py-1.5 rounded-full">TOP 1% IN RETAIL</Badge>
        </Card>
        ...
        <Card className="md:col-span-2 rounded-[2.5rem] border-none shadow-sm bg-card p-10 grid grid-cols-1 sm:grid-cols-2 gap-10 items-center">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {[
                { label: "FSSAI Supermarket License", status: "Active", expiry: "Dec 31, 2024" },
                { label: "Halal Meat Counter Audit", status: "Active", expiry: "Ongoing" },
                { label: "Bakery Compliance Lvl 5", status: "Verified", expiry: "Jun 2025" },
              ].map((item, i) => (
          ...
          <div className="space-y-6 bg-zinc-900 text-white p-8 rounded-[2rem] relative overflow-hidden">
            <Microscope className="absolute -top-4 -right-4 h-24 w-24 opacity-10 text-emerald-600" />
            <h4 className="text-xl font-black relative z-10">Departmental Audits</h4>
            <p className="text-xs text-muted-foreground leading-relaxed relative z-10">
              Generate QR logs for your fresh departments. Shoppers can scan to see verified supplier certificates for meat, dairy, and bakery items.
            </p>
      ...
      <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-card">
        <CardHeader className="p-8 border-b">
          ...
          <p className="text-sm text-muted-foreground font-medium">Historical logs of your store's department-level compliance checks.</p>
        </CardHeader>
              ...
              <div key={log.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-muted/50 transition-colors">
                ...
                <div className="flex items-center gap-10">
                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-1">Audit Score</p>
