    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline text-slate-900">Digital Bills & POS</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 flex flex-col justify-between">
          ...
          <div className="space-y-1">
            <h2 className="text-4xl font-black text-slate-900">₹62,100</h2>
            <p className="text-xs font-bold text-emerald-600 uppercase">74% of total sales</p>
          </div>
        </Card>
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 flex flex-col justify-between">
          <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Cash in Hand</p>
          <div className="space-y-1">
            <h2 className="text-4xl font-black text-slate-900">₹22,150</h2>
      ...
      <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white">
        <CardHeader className="p-8 border-b flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-64">
            ...
            <Input placeholder="Search Invoice ID..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none" />
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-none">
                <TableHead className="px-8 h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Invoice / ID</TableHead>
                <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Customer & Type</TableHead>
            ...
            <TableBody>
              {bills.map((bill) => (
                <TableRow key={bill.id} className="border-slate-100 hover:bg-slate-50/50 transition-colors group">
                  <TableCell className="px-8 py-5">
                    <div className="font-black text-slate-900 text-xs">{bill.id}</div>
                    <div className="text-[10px] font-bold text-muted-foreground uppercase">{bill.time}</div>
                  </TableCell>
                  <TableCell>
                    <p className="font-bold text-slate-800">{bill.customer}</p>
                  ...
                  <TableCell className="font-black text-slate-900">{bill.total}</TableCell>
