...
const CATEGORY_COLOR: Record<string, string> = {
  ...
  "Volunteer Call": "bg-amber-100 text-amber-700",
  "Event":          "bg-purple-100 text-purple-700",
  "Zakat & Sadaqah":"bg-teal-100 text-teal-700",
}
...
export default function OrganizationAnnouncementsPage() {
  ...
  const [form, setForm] = React.useState({ title: "", body: "", category: "General", is_pinned: false })
  ...
  const load = React.useCallback(() => {
    ...
    supabase.from("businesses").select("id").eq("owner_id", user.uid).limit(1).maybeSingle()
      .then(({ data }: { data: { id: string } | null }) => {
        ...
        setOrgId(data.id)
        supabase.from("organization_announcements").select("*")
          .eq("org_id", data.id)
  ...
  async function handleAdd() {
    ...
    setSaving(true)
    await createClient().from("organization_announcements").insert({
      org_id: orgId,
  ...
  async function handleDelete(id: string) {
    await createClient().from("organization_announcements").delete().eq("id", id)
    load()
  }

  async function togglePin(id: string, current: boolean) {
    await createClient().from("organization_announcements").update({ is_pinned: !current }).eq("id", id)
    load()
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Post updates for your community — visible on your organisation listing.</p>
        </div>
          ...
          <CardContent className="pt-5 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1 col-span-2 sm:col-span-1">
                ...
                <Input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Ramadan food drive this Friday" autoFocus />
              </div>
              ...
              <div className="space-y-1 col-span-2">
                ...
                <Textarea value={form.body} onChange={e => setForm(p => ({ ...p, body: e.target.value }))} placeholder="Additional details..." rows={3} className="resize-none rounded-xl" />
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2 ml-auto">
                <Button className="h-9 rounded-lg font-bold" onClick={handleAdd} disabled={saving}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : "Post"}</Button>
      ...
      {items.length === 0 && !adding && (
        <div className="text-center py-16 space-y-3">
          ...
          <p className="text-sm text-muted-foreground">Post updates about programmes, events, and volunteer opportunities.</p>
        </div>
      )}

      {pinned.length > 0 && (
        <div className="space-y-2">
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1.5"><Pin className="h-3 w-3" /> Pinned</p>
      ...
      {recent.length > 0 && (
        <div className="space-y-2">
          {pinned.length > 0 && <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Recent</p>}
...
function AnnouncementCard({ item, onDelete, onPin }: {
  item: Announcement
    ...
    <Card className={item.is_pinned ? "border-primary/30 bg-primary/5" : ""}>
      <CardContent className="p-4 flex items-start gap-3">
        <div className="flex-1 min-w-0 space-y-1.5">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {item.category && item.category !== "General" && (
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${CATEGORY_COLOR[item.category] ?? "bg-muted text-muted-foreground"}`}>{item.category}</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button onClick={() => onPin(item.id, !!item.is_pinned)}
          ...
          <button onClick={() => onDelete(item.id)} className="p-1.5 rounded-lg text-muted-foreground/40 hover:text-destructive transition-colors">
