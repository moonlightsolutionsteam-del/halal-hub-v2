    ...
    <div className="container mx-auto p-6 space-y-8 max-w-5xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline text-slate-900">Parent & Student Feedback</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-6">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-10 text-center space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Global Institutional Score</p>
            </div>
            <div className="pt-6 border-t border-slate-50 space-y-4">
              {[5, 4, 3, 2, 1].map((star) => (
                <div key={star} className="flex items-center gap-4">
                  ...
                  <span className="text-[10px] font-bold text-slate-300 w-8 text-right">{star === 5 ? '85%' : star === 4 ? '10%' : '2%'}</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-slate-900 text-white p-8 space-y-6">
            <div className="flex items-center gap-3">
              <div className={`h-10 w-10 bg-white/10 rounded-xl flex items-center justify-center text-violet-600`}>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 gap-6">
            {reviews.map((rev) => (
              <Card key={rev.id} className="rounded-[2.5rem] border-none shadow-sm bg-white overflow-hidden border-2 border-transparent hover:border-violet-100 transition-all group">
                <div className="p-8 space-y-6">
                  ...
                  {rev.response ? (
                    <div className="p-6 bg-slate-50 rounded-3xl border-l-4 border-violet-600 space-y-2">
