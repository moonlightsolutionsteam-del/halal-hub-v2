    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline text-slate-900">Fees & Grants</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-slate-900 text-white p-10 space-y-10 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10 pt-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase text-slate-400">Term-to-Date</p>
        ...
        <div className="grid grid-rows-2 gap-6">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 flex items-center justify-between group hover:shadow-md transition-all">
            <div className="flex items-center gap-6">
              <div className="h-12 w-12 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-600">
                <Clock className="h-6 w-6" />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Fee Overdue</p>
                <p className="text-xl font-black text-slate-900">₹8,500</p>
              </div>
            </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 flex items-center justify-between group hover:shadow-md transition-all">
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Scholarships Paid</p>
                <p className="text-xl font-black text-slate-900">₹12,400</p>
              </div>
            </div>
            <ArrowUpRight className="h-4 w-4 text-slate-300 group-hover:text-primary transition-colors" />
          </Card>
      ...
      <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white">
        <CardHeader className="p-8 border-b flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-64">
            ...
            <Input placeholder="Search Student or ID..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none font-medium" />
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-none">
                ...
                <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Student & Type</TableHead>
              ...
              {payments.map((p) => (
                <TableRow key={p.id} className="border-slate-100 hover:bg-slate-50/50 transition-colors group">
                  <TableCell className="px-8 py-5 text-xs font-black text-slate-900">
                    {p.id}<br /><span className="text-slate-400 font-bold uppercase">{p.date}</span>
                  </TableCell>
                  <TableCell>
                    <p className="font-bold text-slate-800">{p.student}</p>
