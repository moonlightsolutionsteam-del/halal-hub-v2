    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline">Business Events</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-3xl font-black text-slate-900">3 Events</p>
          </div>
        </Card>
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 flex items-center gap-6">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest leading-none mb-1">RSVPs Received</p>
        ...
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-slate-900 text-white p-8 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest leading-none mb-1">Event Reach</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {events.map((event) => (
          <Card key={event.id} className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl transition-all duration-500 group border-2 border-transparent hover:border-primary/10">
            <div className="relative aspect-video">
              ...
              <Badge className="absolute top-6 left-6 bg-white/90 backdrop-blur-md text-slate-900 font-black border-none uppercase text-[10px] tracking-widest">{event.type}</Badge>
              <div className="absolute bottom-6 left-6">
                <Badge className={event.status === 'Active' ? 'bg-emerald-500 text-white border-none' : 'bg-slate-500 text-white border-none'}>
                  {event.status}
                </Badge>
              </div>
            </div>
        ...
        <button className="rounded-[2.5rem] border-4 border-dashed border-slate-100 bg-slate-50/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-white hover:border-primary/20 transition-all cursor-pointer group min-h-[300px]">
          ...
          <div className="space-y-1">
            <p className="font-black text-xl text-slate-900">Host New Event</p>
            <p className="text-sm text-slate-400 font-medium max-w-[200px]">Promote your restaurant with community sessions.</p>
          </div>
        </button>
      </div>
    </div>
