    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "Completed", value: "142", color: "text-slate-500", bg: "bg-slate-50" },
        ].map((stat, i) => (
          <Card key={i} className="border-none shadow-sm rounded-3xl p-4 flex items-center gap-4 bg-white">
      ...
      <Tabs defaultValue="all" className="space-y-6">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <TabsList className="bg-white border rounded-[1.5rem] h-12 p-1">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <Input placeholder="Search ID or Name..." className="pl-9 h-11 rounded-2xl bg-white border-none shadow-sm" />
            </div>
                    ...
                    <TableRow key={order.id} className="hover:bg-muted/5 border-muted/20">
                      ...
                      <TableCell>
                        <Badge className={
                          ...
                          order.status === 'Pending' ? 'bg-blue-500' : 'bg-slate-500'
                        }>
                          {order.status}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right px-8">
                        <div className="flex items-center justify-end gap-2">
                          <Button size="icon" variant="ghost" className="rounded-xl hover:bg-slate-100"><Eye className="h-4 w-4 text-slate-600" /></Button>
