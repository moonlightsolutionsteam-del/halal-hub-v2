    ...
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl sm:text-3xl font-black font-headline text-foreground">Source Documents</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8">
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 gap-4">
            {documents.map((doc) => (
              <Card key={doc.id} className="rounded-[2rem] border-none shadow-sm bg-card overflow-hidden border-2 border-transparent hover:border-red-100 transition-all group">
                <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center gap-6">
                    <div className="h-14 w-14 rounded-2xl bg-muted flex items-center justify-center text-muted-foreground group-hover:text-red-600 group-hover:bg-red-50 transition-colors shadow-inner">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button size="icon" variant="ghost" className="rounded-xl hover:text-red-600"><Trash2 className="h-4 w-4 text-muted-foreground" /></Button>
                    </div>
                  </div>
                </div>
              </Card>
        ...
        <div className="lg:col-span-4 space-y-8">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-zinc-900 text-white p-10 space-y-8 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground font-medium">Your current document integrity score is high.</p>
              </div>
          ...
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-card p-8 space-y-6">
            <h3 className="text-xl font-black text-foreground">Audit History</h3>
            <div className="space-y-4">
              {[
                { label: "Annual Halal Check", result: "Passed", date: "Jan 2024" },
                { label: "Hygiene Spot Audit", result: "Passed", date: "Dec 2023" },
              ].map((audit, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-muted rounded-2xl border border-transparent hover:border-red-100 transition-all group">
                  <div>
                    <p className="text-sm font-bold text-foreground">{audit.label}</p>
                    <p className="text-[10px] font-black text-muted-foreground uppercase">{audit.date}</p>
                  </div>
                  <Badge variant="secondary" className="bg-card text-emerald-600 font-black text-[9px] uppercase">{audit.result}</Badge>
                </div>
              ))}
            </div>
          </Card>
