...
import {
  Star, Reply, MoreVertical, CheckCircle2
} from "lucide-react";
              ...
              {[5, 4, 3, 2, 1].map((star) => (
                <div key={star} className="flex items-center gap-4">
                  ...
                  <div className="h-2 bg-muted rounded-full flex-1 overflow-hidden">
                    <div className="h-full bg-red-600 rounded-full" style={{ width: `${((distribution[star] ?? 0) / maxCount) * 100}%` }} />
                  </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-12 w-12 border-2 border-border shadow-sm">
                        <AvatarFallback>{(rev.profiles?.name ?? "C")[0]}</AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="text-base font-black text-foreground">{rev.profiles?.name ?? "Customer"}</p>
                  ...
                  {rev.business_response ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <Button onClick={() => submitReply(rev.id)} className="rounded-2xl h-10 px-6 font-black text-xs bg-red-600 text-white">Send Reply</Button>
                        <Button variant="outline" onClick={() => setReplyingTo(null)} className="rounded-2xl h-10 px-6 font-black text-xs">Cancel</Button>
                      </div>
                    </div>
                  ) : (
                    <div className="pt-4 flex gap-3">
                      <Button onClick={() => setReplyingTo(rev.id)} className="rounded-2xl h-11 px-6 font-black uppercase text-[10px] tracking-widest bg-red-600 text-white shadow-lg shadow-red-200">
                        <Reply className="mr-2 h-4 w-4" /> Reply to Customer
                      </Button>
                    </div>
                  )}
                </div>
