...
import {
  Camera, Upload, Trash2, GripVertical,
} from "lucide-react";
...
export default function ButcherGalleryPage() {
  ...
  function removeImage(url: string) {
    saveImages(images.filter(i => i !== url))
  }
  ...
  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-6">
        {images.map((img, i) => (
          <Card key={img} className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden group hover:shadow-xl transition-all border-2 border-transparent hover:border-red-100">
            <div className="relative aspect-video">
              <Image src={img} alt={`Gallery photo ${i + 1}`} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
              {i === 0 && (
                <Badge className="absolute top-4 left-4 bg-red-600 text-white border-none px-3 font-black text-[9px] uppercase tracking-widest shadow-xl">PRIMARY PHOTO</Badge>
              )}
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <Button size="icon" variant="destructive" onClick={() => removeImage(img)} className="rounded-xl h-10 w-10"><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
            {i !== 0 && (
              <div className="p-4 flex items-center justify-end">
                <Button variant="ghost" size="sm" onClick={() => setPrimary(img)} className="text-[10px] font-black uppercase text-red-600 p-0 h-auto hover:bg-transparent hover:underline">Set as primary</Button>
              </div>
            )}
          </Card>
        ))}

        <label className="rounded-[2.5rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-red-200 transition-all cursor-pointer group min-h-[250px]">
          ...
          <div className="space-y-1">
            <p className="font-black text-xl text-foreground">{uploading ? "Uploading…" : "Upload Photo"}</p>
            <p className="text-sm text-muted-foreground font-medium">JPG, PNG, WebP</p>
          </div>
