    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black font-headline text-slate-900">Clinical Services</h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[2rem] shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <Input placeholder="Search services by name or code..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none font-medium" />
        </div>
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="px-4 py-2 rounded-full cursor-pointer border-slate-200">Wellness</Badge>
          <Badge variant="outline" className="px-4 py-2 rounded-full cursor-pointer border-slate-200">Medical</Badge>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
        {services.map((service) => (
          <Card key={service.id} className="group rounded-[3rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl transition-all duration-500 border-2 border-transparent hover:border-teal-100">
            <div className="p-8 flex flex-col sm:flex-row gap-8">
              <div className="relative h-32 w-full sm:w-32 rounded-[2rem] bg-teal-50 flex items-center justify-center shrink-0 shadow-inner group-hover:scale-105 transition-transform duration-700">
                ...
                <Badge className="absolute -top-2 -right-2 bg-white text-teal-600 font-black border-2 border-teal-50 text-[8px] h-6 px-2 flex items-center shadow-lg">NEW</Badge>
              </div>
              <div className="flex-1 space-y-4">
                <div className="space-y-1">
                  ...
                  <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-tight">{service.name}</h3>
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      <Clock className="h-3 w-3" /> Duration: {service.duration}
                    </div>
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Code: {service.code}</span>
                  </div>
                </div>
        ...
        <button className="rounded-[3rem] border-4 border-dashed border-slate-100 bg-slate-50/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-white hover:border-teal-200 transition-all cursor-pointer group min-h-[200px]">
          ...
          <div className="space-y-1">
            <p className="font-black text-xl text-slate-900">Add New Service</p>
            <p className="text-sm text-slate-400 font-medium px-4">Register a new medical or wellness treatment</p>
          </div>
        </button>
      </div>
    </div>
