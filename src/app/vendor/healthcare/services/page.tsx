...
export default function HealthcareServicesPage() {
  ...
  const [form, setForm] = React.useState({
    name: "", category: CATEGORIES[0], description: "", is_free: false, price: "", duration_mins: "", contact: "",
  })

  const load = React.useCallback(() => {
    if (!user?.uid) return
    const supabase = createClient()
    supabase.from("healthcare_services").select("*").eq("vendor_uid", user.uid)
  ...
  async function handleAdd() {
    ...
    setSaving(true)
    const supabase = createClient()
    await supabase.from("healthcare_services").insert({
    ...
    setForm({ name: "", category: CATEGORIES[0], description: "", is_free: false, price: "", duration_mins: "", contact: "" })
    setAdding(false); setSaving(false); load()
  }

  async function handleDelete(id: string) {
    await createClient().from("healthcare_services").delete().eq("id", id)
    load()
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black font-headline">Healthcare Services</h1>
          <p className="text-muted-foreground text-sm">Medical and wellness services offered at your clinic or practice.</p>
        </div>
        ...
        <Card>
          <CardContent className="pt-5 grid grid-cols-2 gap-3">
            <div className="space-y-1 col-span-2 sm:col-span-1">
              ...
              <Input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. General Consultation" />
            </div>
            ...
            <div className="space-y-1 flex items-center gap-3">
              ...
              {!form.is_free && <Input className="w-28 h-9" type="number" placeholder="Fee (₹)" value={form.price} onChange={e => setForm(p => ({ ...p, price: e.target.value }))} />}
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-muted-foreground">Duration (mins)</label>
              <Input type="number" placeholder="e.g. 30" value={form.duration_mins} onChange={e => setForm(p => ({ ...p, duration_mins: e.target.value }))} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-muted-foreground">Contact / Booking</label>
      ...
      {services.length === 0 && !adding && (
        <div className="text-center py-16 space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Add your medical and wellness services to show them on your listing.</p>
        </div>
      )}
            ...
            <CardContent className="p-4 flex items-start gap-3">
              <div className="flex-1 min-w-0 space-y-1">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {s.duration_mins && <span className="text-xs text-muted-foreground">{s.duration_mins} mins</span>}
