...
import { Tag, Plus, Trash2, Edit2, Gift, HeartPulse } from "lucide-react";
...
export default function HealthcareOffersPage() {
  ...
  if (!businessId) {
    return <div className="p-8 text-center text-muted-foreground">Register your clinic to manage offers.</div>
  }
  ...
  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-5xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <Button onClick={() => { setForm(EMPTY_FORM); setShowCreateOfferModal(true) }} className="bg-teal-600 hover:bg-teal-700 rounded-full px-8 font-black shadow-lg shadow-teal-200 h-12 text-white">
      ...
      <div className="space-y-6">
        ...
        {offers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {offers.map((offer) => (
              <Card key={offer.id} className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden border-2 border-transparent hover:border-teal-100 transition-all group">
                <div className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-10">
                  <div className="flex items-center gap-8">
                    <div className="h-20 w-20 rounded-3xl bg-muted flex items-center justify-center text-teal-600 group-hover:scale-110 transition-transform shadow-inner">
                      {offer.discount_type === "bogo" ? <Gift className="h-10 w-10" /> : <HeartPulse className="h-10 w-10" />}
                    </div>
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-4">
                        {offer.code && <span className="text-xs font-black text-teal-600 bg-teal-50 px-3 py-1 rounded-full uppercase tracking-tighter">{offer.code}</span>}
      ...
      <Dialog open={showCreateOfferModal} onOpenChange={setShowCreateOfferModal}>
        <DialogContent className="rounded-[2rem] max-w-lg">
          <DialogHeader><DialogTitle className="text-xl font-black">Create New Offer</DialogTitle><DialogDescription>Set up a new wellness offer for your patients.</DialogDescription></DialogHeader>
          <div className="space-y-4 pt-2">
            <div className="space-y-2"><Label className="font-black text-xs uppercase tracking-widest">Offer Title</Label>
              <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. New Patient Welcome" className="h-12 rounded-2xl bg-muted border-none font-bold" /></div>
            <div className="space-y-2"><Label className="font-black text-xs uppercase tracking-widest">Coupon Code</Label>
              <Input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="e.g. WELCOME20" className="h-12 rounded-2xl bg-muted border-none font-bold" /></div>
            ...
            <Button disabled={saving || !form.title.trim()} className="w-full h-12 rounded-2xl font-black bg-teal-600 hover:bg-teal-700 text-white" onClick={createOffer}>{saving ? "Creating..." : "Create Offer"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editingOffer} onOpenChange={(open) => !open && setEditingOffer(null)}>
        <DialogContent className="rounded-[2rem] max-w-lg">
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-2"><Label className="font-black text-xs uppercase tracking-widest">Expiry Date</Label>
              <Input value={form.valid_until} onChange={e => setForm(f => ({ ...f, valid_until: e.target.value }))} type="date" className="h-12 rounded-2xl bg-muted border-none font-bold" /></div>
            <Button disabled={saving} className="w-full h-12 rounded-2xl font-black bg-teal-600 hover:bg-teal-700 text-white" onClick={saveEdit}>{saving ? "Saving..." : "Save Changes"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deletingOffer} onOpenChange={(open) => !open && setDeletingOffer(null)}>
        <DialogContent className="rounded-[2rem] max-w-sm">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button className="flex-1 h-12 rounded-2xl font-black bg-teal-600 hover:bg-teal-700 text-white" onClick={deleteOffer}>Delete</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
