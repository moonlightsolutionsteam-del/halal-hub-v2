...
export default function HealthcareGalleryPage() {
  ...
  if (!businessId) {
    return <div className="p-8 text-center text-muted-foreground">Register your clinic to manage a gallery.</div>
  }
    ...
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground font-medium">Showcase your clinic's facilities, privacy standards, and equipment.</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 sm:gap-6">
        {images.map((img, i) => (
          <Card key={img} className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden group hover:shadow-xl transition-all border-2 border-transparent hover:border-teal-100">
            <div className="relative aspect-video">
              <Image src={img} alt={`Clinic photo ${i + 1}`} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
              {i === 0 && (
                <Badge className="absolute top-4 left-4 bg-teal-600 text-white border-none px-3 font-black text-[9px] uppercase tracking-widest shadow-xl">COVER PHOTO</Badge>
              )}
            ...
            {i !== 0 && (
              <div className="p-6 flex items-center justify-end">
                <Button variant="ghost" size="sm" onClick={() => setPrimary(img)} className="text-[10px] font-black uppercase text-teal-600 p-0 h-auto hover:bg-transparent hover:underline">Set as cover</Button>
              </div>
            )}
          </Card>
        ))}

        <label className="rounded-[2.5rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-teal-200 transition-all cursor-pointer group min-h-[250px]">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground font-medium">Add facility photos</p>
          </div>
