...
import { 
  ...
  Beaker, FlaskConical, Zap
} from "lucide-react";

export default function CosmeticsTransparencyPage() {
  const auditLogs = [
    { id: "COS-AUD-101", topic: "Breathability Lab Test", status: "Pass", date: "Nov 01, 2024", score: "100%" },
    { id: "COS-AUD-102", topic: "Animal Derivative Audit", status: "Verified", date: "Oct 28, 2024", score: "100%" },
    { id: "COS-AUD-103", topic: "Alcohol-Free Solvent Check", status: "Pass", date: "Oct 15, 2024", score: "98%" },
  ];
    ...
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-rose-600 font-black uppercase tracking-widest text-[10px]">
            <ShieldCheck className="h-3 w-3" /> Purity Hub
          </div>
          <h1 className="text-2xl sm:text-3xl font-black font-headline text-foreground">Purity & Lab Transparency</h1>
          <p className="text-muted-foreground font-medium">Manage your formulation lab reports, ingredient audits, and official halal certifications.</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button className="bg-rose-600 hover:bg-rose-700 rounded-full px-8 font-black shadow-lg shadow-rose-200 h-12 text-white">
            <Award className="mr-2 h-4 w-4" /> Request Lab Audit
          </Button>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-rose-600 text-white p-10 text-center space-y-4">
          <div className="h-20 w-20 bg-card/20 rounded-full flex items-center justify-center mx-auto backdrop-blur-md">
            <ShieldCheck className="h-10 w-10 text-white" />
          </div>
          <div className="space-y-1">
            <h2 className="text-5xl font-black tracking-tighter">100%</h2>
            <p className="text-xs font-bold uppercase tracking-widest opacity-80">Purity Score</p>
          </div>
          <Badge className="bg-card text-rose-600 font-black border-none uppercase text-[9px] px-4 py-1.5 rounded-full">TOP 1% Verified</Badge>
        </Card>
        ...
        <Card className="md:col-span-2 rounded-[2.5rem] border-none shadow-sm bg-card p-10 grid grid-cols-1 sm:grid-cols-2 gap-10 items-center">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {[
                { label: "Breathability Certificate", status: "Active", expiry: "Dec 31, 2024" },
                { label: "Vegan Base Audit", status: "Active", expiry: "Ongoing" },
                { label: "Alcohol-Free Compliance", status: "Verified", expiry: "Jun 2025" },
              ].map((item, i) => (
          ...
          <div className="space-y-6 bg-zinc-900 text-white p-8 rounded-[2rem] relative overflow-hidden">
            <Microscope className="absolute -top-4 -right-4 h-24 w-24 opacity-10 text-rose-600" />
            <h4 className="text-xl font-black relative z-10">Scan-to-Verify</h4>
            <p className="text-xs text-muted-foreground leading-relaxed relative z-10">
              Customers can scan your product barcodes to view real-time lab reports proving the absence of animal derivatives and non-permissible alcohols.
            </p>
      ...
      <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-card">
        <CardHeader className="p-8 border-b">
          ...
          <p className="text-sm text-muted-foreground font-medium">Historical logs of your brand's formulation compliance checks.</p>
        </CardHeader>
