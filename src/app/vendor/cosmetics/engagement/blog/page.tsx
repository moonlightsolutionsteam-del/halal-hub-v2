...
export default function CosmeticsBlogPage() {
  const articles = [
    { id: 1, title: "The Science of Halal Ingredients", status: "Published", date: "Nov 01, 2024", reads: "4.2k", comments: 24, category: "Education" },
    { id: 2, title: "Winter Skincare Guide", status: "Draft", date: "Oct 28, 2024", reads: "0", comments: 0, category: "Beauty Tips" },
    { id: 3, title: "Our Ethical Sourcing Journey", status: "Published", date: "Oct 15, 2024", reads: "8.4k", comments: 156, category: "Brand Story" },
  ];
    ...
    <div className="container mx-auto p-6 space-y-8 max-w-5xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-rose-600 font-black uppercase tracking-widest text-[10px]">
            <BookOpen className="h-3 w-3" /> Beauty Archive
          </div>
          <h1 className="text-3xl font-black font-headline text-slate-900">Brand Blog & Insights</h1>
          <p className="text-muted-foreground font-medium">Educate your audience on ingredient purity, skincare science, and ethical beauty standards.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { label: "Reader Comments", value: "1.5k", icon: MessageSquare },
        ].map((stat, i) => (
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between px-2">
          <div className="relative w-full md:w-80">
            ...
            <Input placeholder="Search articles..." className="pl-9 h-11 rounded-2xl bg-white border-none shadow-sm font-medium" />
          </div>
          <div className="flex items-center gap-3">
            <Badge variant="secondary" className="px-4 py-2 rounded-full cursor-pointer bg-rose-50 text-rose-600 border-none">Published</Badge>
        ...
        <div className="grid grid-cols-1 gap-6">
          {articles.map((article) => (
            <Card key={article.id} className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-xl transition-all border-2 border-transparent hover:border-rose-100">
