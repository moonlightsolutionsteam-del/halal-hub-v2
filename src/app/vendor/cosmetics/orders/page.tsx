...
export default function CosmeticsOrdersPage() {
  const orders = [
    { id: "COS-9021", customer: "Amara Siddiqui", items: "1x Glow Serum, 2x Rose Water", total: "₹3,150", status: "Packing", time: "15 mins ago", type: "Standard Ship" },
    { id: "COS-9022", customer: "Layla Khan", items: "1x Mineral Foundation", total: "₹2,100", status: "Shipped", time: "1 hour ago", type: "Express Ship" },
    { id: "COS-9023", customer: "Zainab Ali", items: "3x Lip Tint Bundle", total: "₹1,800", status: "Ready", time: "2 hours ago", type: "Store Pickup" },
  ];
    ...
    <div className="container mx-auto p-6 space-y-8 max-w-6xl pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-rose-600 font-black uppercase tracking-widest text-[10px]">
            <ShoppingBag className="h-3 w-3" /> Retail Fulfillment
          </div>
          <h1 className="text-3xl font-black font-headline text-slate-900">Manage Orders</h1>
        ...
        <div className="flex gap-3">
          ...
          <Button className="bg-rose-600 hover:bg-rose-700 rounded-full px-8 font-black shadow-lg shadow-rose-200 h-12 text-white">
            <PackageCheck className="mr-2 h-4 w-4" /> Batch Process
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "In Transit", value: "28", icon: Truck, color: "text-purple-600", bg: "bg-purple-50" },
          { label: "Avg Fulfillment", value: "2h", icon: Timer, color: "text-emerald-600", bg: "bg-emerald-50" },
        ].map((stat, i) => (
      ...
      <Card className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white">
        <CardHeader className="p-8 border-b flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-64">
            ...
            <Input placeholder="Search Order ID..." className="pl-9 h-11 rounded-2xl bg-slate-50 border-none font-medium" />
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-none">
                ...
                <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400">Customer & Items</TableHead>
              ...
              {orders.map((order) => (
                <TableRow key={order.id} className="border-slate-100 hover:bg-slate-50/50 transition-colors group">
                  ...
                  <TableCell>
                    ...
                    <div className="text-[10px] font-black text-rose-600 uppercase italic line-clamp-1">{order.items}</div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right px-8">
                    <Badge variant="outline" className={
                      ...
                      'bg-slate-50 text-slate-600 border-slate-200 font-black text-[9px] px-3'
                    }>
                      {order.status}
                    </Badge>
                  </TableCell>
