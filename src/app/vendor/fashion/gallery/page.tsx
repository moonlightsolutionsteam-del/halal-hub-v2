    ...
    <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-6 sm:space-y-8 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl sm:text-3xl font-black font-headline text-foreground">Lookbooks & Media</h1>
          <p className="text-muted-foreground font-medium">Manage your brand lookbooks, seasonal campaigns, and high-fidelity product photography.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" className="rounded-full px-6 font-bold border-2 h-12">
            <Layout className="mr-2 h-4 w-4" /> Portfolio Grid
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8">
        <div className="lg:col-span-8 space-y-8">
          <div className="grid grid-cols-2 gap-3 sm:gap-6">
            {photos.map((photo) => (
              <Card key={photo.id} className="rounded-[2.5rem] border-none shadow-sm bg-card overflow-hidden group hover:shadow-xl transition-all border-2 border-transparent hover:border-pink-100">
                <div className="relative aspect-[3/4]">
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    {photo.primary && (
                      <Badge className="bg-pink-600 text-white border-none px-3 font-black text-[9px] uppercase tracking-widest shadow-xl">BRAND COVER</Badge>
                    )}
                ...
                <div className="p-6 flex items-center justify-between">
                  ...
                  {!photo.primary && (
                    <Button variant="ghost" size="sm" className="text-[10px] font-black uppercase text-pink-600 p-0 h-auto hover:bg-transparent hover:underline">Set as Brand Primary</Button>
                  )}
                </div>
              </Card>
            ))}
            
            <button className="rounded-[2.5rem] border-4 border-dashed border-border bg-muted/30 flex flex-col items-center justify-center p-12 text-center gap-4 hover:bg-card hover:border-pink-200 transition-all cursor-pointer group min-h-[350px]">
              ...
              <div className="space-y-1">
                <p className="font-black text-xl text-foreground">Upload Visuals</p>
                <p className="text-sm text-muted-foreground font-medium px-4">Add high-fidelity editorial shots or product reels</p>
              </div>
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <Card className="rounded-[2.5rem] border-none shadow-sm bg-zinc-900 text-white p-8 space-y-6">
            ...
            <p className="text-sm text-muted-foreground font-medium leading-relaxed">
              Modest fashion brands with editorial-style lookbooks see 5x more customer saves. Ensure your silhouette and fabric quality is clearly visible.
            </p>
            <div className="space-y-4">
              <div className="flex items-center gap-3 bg-card/5 p-4 rounded-2xl border border-white/10">
                <CheckCircle2 className="h-5 w-5 text-emerald-400" />
                <span className="text-xs font-bold text-white/80">HD Campaign Visuals Active</span>
              </div>
              <div className="flex items-center gap-3 bg-card/5 p-4 rounded-2xl border border-white/10">
                <Shirt className="h-5 w-5 text-pink-400" />
                <span className="text-xs font-bold text-white/80">4 Active Collections Pictured</span>
              </div>
            </div>
          </Card>
        </div>
