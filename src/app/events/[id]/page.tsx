    ...
    <div className="flex flex-col min-h-screen bg-[#FBFBFB] pb-24 selection:bg-primary/10">
      ...
      <div className="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b">
        <div className="container mx-auto max-w-7xl px-6 h-20 flex items-center justify-between">
          <Button 
            variant="ghost" 
            className="flex items-center gap-2 text-sm font-black text-slate-600 hover:text-primary transition-all group p-0 h-auto"
            onClick={() => router.back()}
          >
            ...
            <Tabs defaultValue="about" className="w-full">
              ...
              <TabsContent value="about" className="mt-12 space-y-12 animate-in fade-in duration-700">
                <div className="space-y-6">
                  <h2 className="text-3xl font-black tracking-tight text-slate-900">Event Overview</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 space-y-4">
                    <h3 className="text-sm font-black uppercase text-slate-400 tracking-widest">Target Audience</h3>
                    <div className="flex items-center gap-4">
                      <div className="h-12 w-12 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600">
                        <Users className="h-6 w-6" />
                      </div>
                      <p className="text-lg font-black text-slate-900">Professionals & Public</p>
                    </div>
                  </Card>
                  <Card className="rounded-[2.5rem] border-none shadow-sm bg-white p-8 space-y-4">
                    <h3 className="text-sm font-black uppercase text-slate-400 tracking-widest">Estimated Capacity</h3>
                    <div className="flex items-center gap-4">
                      <div className="h-12 w-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600">
                        <CheckCircle2 className="h-6 w-6" />
                      </div>
                      <p className="text-lg font-black text-slate-900">{event.attendees} Pax</p>
                    </div>
                  </Card>
                </div>

                <div className="space-y-6">
                  <h3 className="text-2xl font-black text-slate-900">On-site Amenities</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {event.amenities.map((item: any, i: number) => (
                      <div key={i} className="flex flex-col items-center justify-center p-6 bg-white rounded-3xl border border-slate-100 shadow-sm group hover:border-primary/20 transition-all">
              ...
              <TabsContent value="schedule" className="mt-12 animate-in fade-in duration-700">
                <div className="space-y-6">
                  {event.schedule.map((item: any, i: number) => (
                    <div key={i} className="flex items-start gap-8 p-8 bg-white rounded-[2.5rem] shadow-sm border border-slate-50 group hover:shadow-lg transition-all">
                      <div className="text-center w-24 shrink-0">
                        <p className="text-xl font-black text-primary">{item.time.split(' ')[0]}</p>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.time.split(' ')[1]}</p>
                      </div>
                      <div className="h-12 w-px bg-slate-100" />
                      <div className="space-y-1">
                        <h4 className="text-xl font-black text-slate-900 leading-tight group-hover:text-primary transition-colors">{item.event}</h4>
                        <p className="text-sm font-bold text-slate-400">Conference Hall A • Main Wing</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <TabsContent value="speakers" className="mt-12 animate-in fade-in duration-700">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  {[1, 2].map((i) => (
                    <Card key={i} className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-white p-8 space-y-6 hover:shadow-xl transition-all border-2 border-transparent hover:border-primary/10">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <h4 className="text-xl font-black text-slate-900">Dr. Abdullah {i === 1 ? "Hussain" : "Ali"}</h4>
          ...
          <div className="lg:col-span-4 space-y-10">
            <Card className="rounded-[3rem] border-none shadow-2xl overflow-hidden bg-white sticky top-28 border border-slate-100">
              ...
              <CardContent className="p-10 space-y-8">
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-6 bg-slate-50 rounded-[2rem] border-2 border-transparent hover:border-primary/10 transition-all">
                    <div className="space-y-1">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">General Access</p>
                      <p className="text-2xl font-black text-slate-900">{event.currency}{event.price}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-8 w-8 rounded-lg bg-white shadow-sm border"
                ...
                <div className="pt-8 border-t border-slate-50 space-y-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-black text-slate-900 uppercase">Secure Transaction</p>
                      <p className="text-[10px] font-medium text-slate-400 leading-relaxed">Your data is end-to-end encrypted and handled with theological integrity.</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="rounded-[3rem] border-none bg-slate-900 p-10 text-center space-y-6 relative overflow-hidden">
              <div className="absolute -top-4 -right-4 h-24 w-24 bg-primary/10 rounded-full blur-2xl" />
              <div className="h-16 w-16 bg-white/10 rounded-2xl flex items-center justify-center mx-auto border border-white/10 shadow-2xl">
              ...
              <Button variant="outline" className="w-full h-12 rounded-2xl border-white/20 text-white hover:bg-white/10 font-black text-xs uppercase tracking-widest">
                Chat with Concierge
              </Button>
            </Card>
          </div>
