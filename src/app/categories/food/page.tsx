        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-4">
              <div className="h-14 w-14 rounded-2xl flex items-center justify-center bg-orange-100 text-orange-600 shadow-inner">
                <Utensils className="h-8 w-8" />
              </div>
              <div className="space-y-1">
                <h1 className="text-5xl font-black font-headline text-slate-900 tracking-tight">Food & Dining</h1>
                <p className="text-muted-foreground font-medium text-xl">Verified halal eateries, from hidden gems to Michelin stars.</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-96">
              ...
              <Input placeholder="Search cuisines, dishes, or restaurant names..." className="pl-12 h-14 rounded-2xl bg-white border-none shadow-sm font-medium text-lg" />
            </div>
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all whitespace-nowrap shadow-sm border-2 ${
              selectedCuisine === cuisine 
                ...
                : "bg-white text-slate-600 border-transparent hover:border-orange-200"
            }`}
          >
            {cuisine}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-md p-8 bg-white space-y-8 sticky top-24">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <p className="text-xs font-black uppercase text-slate-900 tracking-widest">Filter by status</p>
                <div className="space-y-3">
                  {["Verified Only", "Open Now", "Top Rated"].map(f => (
                    <label key={f} className="flex items-center gap-3 cursor-pointer group">
                      <div className="h-5 w-5 border-2 rounded-lg border-slate-200 group-hover:border-primary transition-colors flex items-center justify-center">
              ...
              <div className="space-y-4">
                <p className="text-xs font-black uppercase text-slate-900 tracking-widest">Price Range</p>
                <div className="flex gap-2">
                  {["$", "$$", "$$$", "$$$$"].map(p => (
                    <button key={p} className="flex-1 py-2 rounded-xl bg-slate-50 text-slate-400 font-black text-xs hover:bg-orange-50 hover:text-orange-600 transition-colors border border-transparent hover:border-orange-100">
                      {p}
                    </button>
                  ))}
                </div>
        ...
        <div className="lg:col-span-9 space-y-8">
          <div className="flex items-center justify-between px-2">
            <p className="text-sm font-bold text-muted-foreground tracking-tight">Found <span className="text-slate-900">{MOCK_RESTAURANTS.length}</span> restaurants in <span className="text-primary">{selectedCuisine}</span></p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-8">
            {MOCK_RESTAURANTS.map((res) => (
              <Link key={res.id} href={`/entities/${res.id}`}>
                <Card className="group rounded-[3rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-2xl transition-all duration-700 flex flex-col h-full border-2 border-transparent hover:border-orange-100/50">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2">
                      <div className="flex gap-2">
                        ...
                        <Badge className="bg-slate-900/80 backdrop-blur-md text-white font-black border-none shadow-xl px-4 py-1.5 rounded-full">
                          {res.price}
                        </Badge>
                      </div>
                    </div>
                    {res.ver && (
                      <div className="absolute bottom-6 left-6">
                        <Badge className="bg-emerald-500 text-white font-black border-none shadow-xl px-5 py-2 rounded-full uppercase text-[10px] tracking-widest flex items-center gap-2">
                          <div className="h-2 w-2 bg-white rounded-full animate-pulse" /> Verified Halal
                        </Badge>
                      </div>
                    )}
                  </div>
                  ...
                  <CardHeader className="p-8 pb-4">
                    <div className="space-y-2">
                      ...
                      <CardTitle className="text-3xl font-black group-hover:text-primary transition-colors leading-tight">{res.name}</CardTitle>
                  ...
                  <CardContent className="px-8 pb-8 flex-1 space-y-6">
                    <div className="flex flex-wrap gap-2">
                      {res.tags.map(tag => (
                        <span key={tag} className="text-[10px] font-bold bg-slate-50 text-slate-500 px-3 py-1 rounded-lg">#{tag}</span>
                      ))}
                    </div>
                  ...
                  <CardFooter className="px-8 pb-8 pt-0 mt-auto">
                    <Button className="w-full bg-primary hover:bg-primary/90 text-white rounded-[1.5rem] font-black text-sm uppercase tracking-widest h-16 shadow-2xl shadow-primary/20 group-hover:scale-[1.02] transition-transform">
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-6">
            ...
            <Button variant="outline" className="rounded-full px-16 font-black border-2 h-16 hover:bg-primary hover:text-white hover:border-primary transition-all text-lg shadow-sm">Explore More Districts</Button>
          </div>
        </div>
      </div>
    </div>
