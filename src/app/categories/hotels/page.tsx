...
export default function HotelsListingPage() {
  ...
  const filtered = useFilteredItems(items, search, selectedAmenity, "All Amenities")
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 sm:gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="h-9 w-9 sm:h-14 sm:w-14 rounded-xl sm:rounded-2xl flex items-center justify-center bg-sky-100 text-sky-600 shadow-inner">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-96">
              ...
              <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search locations, hotel names, or amenities..." className="pl-10 sm:pl-12 h-10 sm:h-14 rounded-xl sm:rounded-2xl bg-card border-none shadow-sm font-medium text-sm sm:text-lg" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-9 space-y-8">
          <div className="flex items-center justify-between px-2">
            <p className="text-sm font-bold text-muted-foreground tracking-tight">Found <span className="text-foreground">{filtered.length}</span> verified properties</p>
                ...
                <Card className="group rounded-2xl sm:rounded-[3rem] border-none shadow-sm overflow-hidden bg-card hover:shadow-2xl transition-all duration-700 flex flex-col h-full border-2 border-transparent hover:border-sky-100/50">
                  <div className="relative aspect-square sm:aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute bottom-2 left-2 sm:bottom-6 sm:left-6 flex gap-2">
                      ...
                      <Badge className="bg-card text-sky-600 font-black border-none shadow-xl px-2 py-1 sm:px-5 sm:py-2 rounded-full uppercase text-[10px] tracking-widest flex items-center gap-1 sm:gap-2">
                        From {hotel.price}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <CardHeader className="p-3 pb-1 sm:p-8 sm:pb-4">
                    <div className="space-y-2">
                      ...
                      <CardTitle className="text-sm sm:text-3xl font-black group-hover:text-sky-600 transition-colors leading-tight">{hotel.name}</CardTitle>
                  ...
                  <CardFooter className="px-3 pb-3 pt-0 sm:px-8 sm:pb-8 mt-auto">
                    <Button className="w-full bg-zinc-900 hover:bg-sky-600 text-white rounded-xl sm:rounded-[1.5rem] font-black text-[10px] sm:text-sm uppercase tracking-widest h-9 sm:h-16 shadow-lg sm:shadow-2xl transition-all group-hover:scale-[1.02]">
          ...
          <div className="flex flex-col items-center justify-center py-6 sm:py-16 gap-4 sm:gap-6">
            ...
            <Button variant="outline" className="rounded-full px-8 sm:px-16 font-black border-2 h-10 sm:h-16 hover:bg-sky-600 hover:text-white hover:border-sky-600 transition-all text-sm sm:text-lg shadow-sm">Explore Global Partners</Button>
          </div>
        </div>
      </div>
    </div>
