...
export default function FashionListingPage() {
  ...
  const items = useCategoryBusinesses("Fashion & Modest Wear", FALLBACK, (b) => ({
    id: b.id, name: b.name, type: b.subcategory, loc: b.city,
    rate: b.rating, ver: b.halal_verified, img: b.image_url, features: b.features,
    startingPrice: b.price_range ?? "Contact", shipping: "Worldwide",
  }))
  const filtered = useFilteredItems(items, search, selectedStyle, "All Styles")
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 sm:gap-8">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-96">
              ...
              <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search designers, styles, or labels..." className="pl-10 sm:pl-12 h-10 sm:h-14 rounded-xl sm:rounded-2xl bg-card border-none shadow-sm font-medium text-sm sm:text-lg" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-9 space-y-8">
          <div className="flex items-center justify-between px-2">
            <p className="text-sm font-bold text-muted-foreground tracking-tight">Found <span className="text-foreground">{filtered.length}</span> verified designers</p>
