        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-4">
              <div className="h-14 w-14 rounded-2xl flex items-center justify-center bg-rose-100 text-rose-600 shadow-inner">
                <Sparkles className="h-8 w-8" />
              </div>
              <div className="space-y-1">
                <h1 className="text-5xl font-black font-headline text-slate-900 tracking-tight">Cosmetics & Beauty</h1>
                <p className="text-muted-foreground font-medium text-xl">Ethical, lab-verified, and 100% halal beauty formulations.</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-96">
              ...
              <Input placeholder="Search serums, lipsticks, or brands..." className="pl-12 h-14 rounded-2xl bg-white border-none shadow-sm font-medium text-lg" />
            </div>
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all whitespace-nowrap shadow-sm border-2 ${
              selectedType === type 
                ...
                : "bg-white text-slate-600 border-transparent hover:border-rose-200"
            }`}
          >
            {type}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-md p-8 bg-white space-y-8 sticky top-24">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <p className="text-xs font-black uppercase text-slate-900 tracking-widest">Trust Badges</p>
                <div className="space-y-3">
                  {["Alcohol Free", "Lab Verified", "Cruelty Free", "Wudu Friendly"].map(f => (
                    <label key={f} className="flex items-center gap-3 cursor-pointer group">
                      <div className="h-5 w-5 border-2 rounded-lg border-slate-200 group-hover:border-rose-600 transition-colors flex items-center justify-center">
              ...
              <div className="space-y-4">
                <p className="text-xs font-black uppercase text-slate-900 tracking-widest">Skin Type</p>
                <div className="grid grid-cols-2 gap-2">
                  {["Dry", "Oily", "Combination", "Sensitive"].map(p => (
                    <button key={p} className="py-2 rounded-xl bg-slate-50 text-slate-400 font-black text-xs hover:bg-rose-50 hover:text-rose-600 transition-colors border border-transparent hover:border-rose-100">
                      {p}
                    </button>
                  ))}
                </div>
            ...
            <Card className="rounded-3xl border-none bg-rose-900 text-white p-8 space-y-4 relative overflow-hidden">
              ...
              <Button variant="secondary" className="w-full rounded-2xl font-black text-xs h-12 shadow-xl bg-white text-rose-900 hover:bg-rose-50">Launch Scanner</Button>
            </Card>
          </Card>
        </aside>
        ...
        <div className="lg:col-span-9 space-y-8">
          <div className="flex items-center justify-between px-2">
            <p className="text-sm font-bold text-muted-foreground tracking-tight">Found <span className="text-slate-900">{MOCK_COSMETICS_BRANDS.length}</span> verified brands</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {MOCK_COSMETICS_BRANDS.map((brand) => (
              <Link key={brand.id} href={`/entities/${brand.id}`}>
                <Card className="group rounded-[3rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-2xl transition-all duration-700 flex flex-col h-full border-2 border-transparent hover:border-rose-100/50">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2">
                      <Badge className="bg-white/90 backdrop-blur-md text-rose-600 font-black border-none shadow-xl px-4 py-1.5 rounded-full flex items-center gap-1.5">
                    ...
                    <div className="absolute bottom-6 left-6 flex gap-2">
                      ...
                      <Badge className="bg-white text-rose-600 font-black border-none shadow-xl px-5 py-2 rounded-full uppercase text-[10px] tracking-widest flex items-center gap-2">
                  ...
                  <CardHeader className="p-8 pb-4">
                    <div className="space-y-2">
                      ...
                      <CardTitle className="text-3xl font-black group-hover:text-rose-600 transition-colors leading-tight">{brand.name}</CardTitle>
                  ...
                  <CardContent className="px-8 pb-8 flex-1 space-y-6">
                    <div className="space-y-3">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Brand Values</p>
                  ...
                  <CardFooter className="px-8 pb-8 pt-0 mt-auto">
                    <Button className="w-full bg-slate-900 hover:bg-rose-600 text-white rounded-[1.5rem] font-black text-sm uppercase tracking-widest h-16 shadow-2xl transition-all group-hover:scale-[1.02]">
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-6">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">End of Brand List</p>
              <div className="h-1 w-12 bg-slate-200 rounded-full" />
            </div>
            <Button variant="outline" className="rounded-full px-16 font-black border-2 h-16 hover:bg-rose-600 hover:text-white hover:border-rose-600 transition-all text-lg shadow-sm">Explore Global Boutiques</Button>
          </div>
        </div>
      </div>
    </div>
