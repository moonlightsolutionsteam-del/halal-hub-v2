        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-3">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                <h1 className="text-5xl font-black font-headline text-slate-900 tracking-tight">Grocery & Supermarkets</h1>
                <p className="text-muted-foreground font-medium text-xl">Fresh produce, halal essentials, and ethcial pantry staples.</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 md:w-96">
              ...
              <Input placeholder="Search for items, brands, or stores..." className="pl-12 h-14 rounded-2xl bg-white border-none shadow-sm font-medium text-lg" />
            </div>
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all whitespace-nowrap shadow-sm border-2 ${
              selectedDept === dept 
                ...
                : "bg-white text-slate-600 border-transparent hover:border-emerald-200"
            }`}
          >
            {dept}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <Card className="rounded-[2.5rem] border-none shadow-md p-8 bg-white space-y-8 sticky top-24">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="font-black text-sm uppercase tracking-widest text-slate-400">Filter Store</h3>
              ...
              <div className="space-y-4">
                <p className="text-xs font-black uppercase text-slate-900 tracking-widest">Service Options</p>
                <div className="space-y-3">
                  {["Delivery Available", "Click & Collect", "In-store Only", "24/7 Open"].map(f => (
                    <label key={f} className="flex items-center gap-3 cursor-pointer group">
                      <div className="h-5 w-5 border-2 rounded-lg border-slate-200 group-hover:border-emerald-600 transition-colors flex items-center justify-center">
              ...
              <div className="space-y-4">
                <p className="text-xs font-black uppercase text-slate-900 tracking-widest">Special Sections</p>
                <div className="flex flex-col gap-2">
                  <button className="flex items-center gap-3 w-full p-3 rounded-xl bg-slate-50 text-slate-600 font-bold text-sm hover:bg-emerald-50 hover:text-emerald-600 transition-colors">
            ...
            <Card className="rounded-3xl border-none bg-emerald-900 text-white p-8 space-y-4 relative overflow-hidden">
              ...
              <Button variant="secondary" className="w-full rounded-2xl font-black text-xs h-12 shadow-xl bg-white text-emerald-900 hover:bg-emerald-50">Join Program</Button>
            </Card>
          </Card>
        </aside>
        ...
        <div className="lg:col-span-9 space-y-8">
          <div className="flex items-center justify-between px-2">
            <p className="text-sm font-bold text-muted-foreground tracking-tight">Found <span className="text-slate-900">{MOCK_GROCERIES.length}</span> verified supermarkets</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {MOCK_GROCERIES.map((store) => (
              <Link key={store.id} href={`/entities/${store.id}`}>
                <Card className="group rounded-[3rem] border-none shadow-sm overflow-hidden bg-white hover:shadow-2xl transition-all duration-700 flex flex-col h-full border-2 border-transparent hover:border-emerald-100/50">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col gap-2">
                      <Badge className="bg-white/90 backdrop-blur-md text-emerald-600 font-black border-none shadow-xl px-4 py-1.5 rounded-full flex items-center gap-1.5">
                    ...
                    <div className="absolute bottom-6 left-6 flex gap-2">
                      ...
                      {store.delivery !== "Not Available" && (
                        <Badge className="bg-white text-emerald-600 font-black border-none shadow-xl px-5 py-2 rounded-full uppercase text-[10px] tracking-widest flex items-center gap-2">
                  ...
                  <CardHeader className="p-8 pb-4">
                    <div className="space-y-2">
                      ...
                      <CardTitle className="text-3xl font-black group-hover:text-emerald-600 transition-colors leading-tight">{store.name}</CardTitle>
                  ...
                  <CardFooter className="px-8 pb-8 pt-0 mt-auto">
                    <Button className="w-full bg-slate-900 hover:bg-emerald-600 text-white rounded-[1.5rem] font-black text-sm uppercase tracking-widest h-16 shadow-2xl transition-all group-hover:scale-[1.02]">
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-6">
            ...
            <Button variant="outline" className="rounded-full px-16 font-black border-2 h-16 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all text-lg shadow-sm">Show International Stores</Button>
          </div>
        </div>
      </div>
    </div>
