          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className="p-16 space-y-10 flex flex-col justify-center">
              <div className="space-y-4">
                ...
                <p className="text-xl text-muted-foreground font-medium leading-relaxed line-clamp-3 italic">"{FEATURED_POST.excerpt}"</p>
              </div>
              <div className="flex items-center justify-between pt-10 border-t border-border">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-lg font-black text-foreground">{FEATURED_POST.author}</p>
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{FEATURED_POST.date}</p>
                  </div>
                </div>
          ...
          {BLOG_POSTS.map((post) => (
            <Card key={post.id} className="rounded-[2.5rem] border-none shadow-sm overflow-hidden bg-card hover:shadow-2xl transition-all duration-500 group flex flex-col h-full border border-transparent hover:border-primary/10 cursor-pointer">
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <Badge className="absolute top-4 left-4 bg-card/90 backdrop-blur-md text-primary font-black border-none px-3 py-1 rounded-full text-[9px] uppercase tracking-widest shadow-lg">{post.cat}</Badge>
              </div>
      ...
      <Card className="rounded-[4rem] border-none bg-zinc-900 text-white p-16 text-center space-y-10 relative overflow-hidden shadow-2xl">
        ...
        <div className="max-w-2xl mx-auto space-y-6 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-6">
            ...
            <Button variant="outline" className="h-16 px-12 rounded-2xl border-white/20 text-white hover:bg-card/10 font-black uppercase text-sm tracking-widest">
              Creator Guidelines
            </Button>
          </div>
        </div>
